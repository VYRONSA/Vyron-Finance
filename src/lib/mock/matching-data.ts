...
export const MOCK_CUSTOMER_MATCHING: CustomerMatchingWorkspaceData = {
  ...
  customers: MOCK_CUSTOMERS.map((c) => ({ id: c.id, name: c.name, vatNumber: c.vatNumber, registrationNumber: c.registrationNumber })),
};
...
export const MOCK_SUPPLIER_MATCHING: SupplierMatchingWorkspaceData = {
  ...
  suppliers: MOCK_SUPPLIERS.map((s) => ({ id: s.id, name: s.name })),
};
