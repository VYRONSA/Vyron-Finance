...
export default async function InternalBillingConsolePage() {
  ...
  // Finding #126 (RC-16/E13) — same pre-fetched invoice-lines-by-id
  // pattern as the per-company Billing tab's own fix, applied to every
  // invoice across the platform (this page already fans out per-account
  // the same way for invoices/payments/credits above).
  const invoiceLinesById = Object.fromEntries(
    await Promise.all(invoices.map(async (inv) => [inv.id, await invoiceRepo.listInvoiceLines(inv.id)] as const)),
  );
