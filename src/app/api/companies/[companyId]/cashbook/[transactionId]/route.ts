...
import { approveAndPostCashbookEntry, cancelCashbookEntry, editCashbookEntry, reverseCashbookEntry, submitCashbookEntry, ValidationError, NotFoundError } from "@/server/services/cashbook-service";
...
export async function PATCH(request: Request, { params }: { params: Promise<{ companyId: string; transactionId: string }> }) {
  ...
  try {
    ...
    if (action === "edit") return NextResponse.json({ transaction: await editCashbookEntry(companyId, id, body) });
