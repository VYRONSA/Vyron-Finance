...
import { resolveException, reopenException, NotFoundError, ValidationError } from "@/server/services/banking-exception-service";
...
export async function PATCH(request: Request, { params }: { params: Promise<{ companyId: string; exceptionId: string }> }) {
  ...
  try {
    if (body.status === "Open") {
      const exception = await reopenException(companyId, Number(exceptionId));
