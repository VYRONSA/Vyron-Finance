...
export async function GET(request: Request, { params }: { params: Promise<{ companyId: string }> }) {
  ...
  // Finding #062 (RC-16) — this log spans every module's outbound
  // communications (subjects, recipients, bodies), so it's gated the
  // same way an uncategorized communication's own Create action already
  // is (`resolveCommunicationPermissionModule`'s own default).
  const check = await requirePermission(companyId, "Settings:View");
