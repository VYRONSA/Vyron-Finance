...
export async function GET(_request: Request, { params }: { params: Promise<{ companyId: string }> }) {
  ...
  // Finding #124 (RC-16) — the full role roster (including each role's
  // resolved permission grants) is gated the same as the mutations
  // below it; a member's own role/permissions stay available via the
  // separate `/roles/me` self-service endpoint regardless.
