...
import { allocateReceipt, unallocateReceipt, NotFoundError, ValidationError } from "@/server/services/customer-receipt-service";
 ...
 * amount onto the invoice's outstanding balance. `?allocationId=` since
 * a receipt can carry several allocations and this removes exactly one. */
export async function DELETE(request: Request, { params }: { params: Promise<{ companyId: string; receiptId: string }> }) {
  const session = await requireSession();
  if (!session.ok) return session.response;
  ...
  try {
    await unallocateReceipt(companyId, Number(allocationId), performedBy);
