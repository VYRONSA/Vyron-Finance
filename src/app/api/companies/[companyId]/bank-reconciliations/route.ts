...
export async function POST(request: Request, { params }: { params: Promise<{ companyId: string }> }) {
  ...
  const { bankAccountId, statementPeriodStart, statementDate, statementOpeningBalance, statementClosingBalance } = body;

  const check = await requirePermission(companyId, "Banking:Create");
  if (!check.ok) return check.response;

  try {
    const reconciliation = await startReconciliation(companyId, {
      bankAccountId: Number(bankAccountId),
      statementPeriodStart: statementPeriodStart ?? null,
      statementDate,
