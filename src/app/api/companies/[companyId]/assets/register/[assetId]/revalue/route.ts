...
import { revalueAsset, ValidationError } from "@/server/services/asset-register-service";
...
export async function POST(request: Request, { params }: { params: Promise<{ companyId: string; assetId: string }> }) {
  ...
  try {
    const asset = await revalueAsset(companyId, Number(assetId), Number(body.delta), body.reason, performedBy);
