...
import { capitaliseAsset, ValidationError } from "@/server/services/asset-register-service";
...
export async function POST(request: Request, { params }: { params: Promise<{ companyId: string; assetId: string }> }) {
  ...
  try {
    const asset = await capitaliseAsset(companyId, Number(assetId), body.inServiceDate, performedBy);
