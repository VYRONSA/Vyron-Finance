...
export async function PATCH(request: Request, { params }: { params: Promise<{ companyId: string; orderId: string }> }) {
  ...
  try {
    switch (body.action) {
      ...
      case "clone-as-draft": {
        const cloneCheck = await requirePermission(companyId, "Purchasing:Create");
        if (!cloneCheck.ok) return cloneCheck.response;
        const order = await cloneOrderAsDraft(companyId, id);
