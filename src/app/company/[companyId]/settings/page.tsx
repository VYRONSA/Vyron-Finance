...
export default async function CompanySettingsPage({ params }: { params: Promise<{ companyId: string }> }) {
  ...
  const [branding, financialYears, branches, departments, costCentres, projects, currencies, companyCurrencies, vatTreatments, roles, roleAssignments] = previewMode
    ? [MOCK_COMPANY_BRANDING, MOCK_FINANCIAL_YEARS, MOCK_BRANCHES, MOCK_DEPARTMENTS, MOCK_COST_CENTRES, MOCK_PROJECTS, MOCK_CURRENCIES, MOCK_COMPANY_CURRENCIES, MOCK_VAT_TREATMENTS, MOCK_PERMISSION_ROLES, MOCK_ROLE_ASSIGNMENTS]
    : await Promise.all([
        getCompanyBrandingAssets(companyId),
