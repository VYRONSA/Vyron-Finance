...
import { getCompany, listCompaniesForUser } from "@/server/services/company-service";
...
export default async function CompanyLayout({
  ...
  if (!isSupabaseConfigured()) {
    return (
      <FinancialWorkspaceShell companyId={companyId} companyName={MOCK_COMPANY.name} companies={MOCK_COMPANIES_FULL} previewMode>
        {children}
  ...
  const [company, companies] = await Promise.all([getCompany(companyId), listCompaniesForUser(userId)]);
  if (!company) notFound();

  return (
    <FinancialWorkspaceShell companyId={companyId} companyName={company.name} companies={companies} userEmail={data.claims.email as string | undefined}>
      {children}
    </FinancialWorkspaceShell>
  );
}
