...
import { listManualJournalHeaders } from "@/server/services/journal-workflow-service";
import { listCompanyMembersWithEmail } from "@/server/services/user-invite-service";
...
import { MOCK_AUDIT_ENGAGEMENTS, MOCK_AUDIT_AREAS, MOCK_AUDIT_PROGRAMME_STEPS, MOCK_AUDIT_RISK_REGISTER, MOCK_AUDIT_TEAM_ASSIGNMENTS, MOCK_AUDIT_FINDINGS, MOCK_AUDIT_WORKING_PAPERS, MOCK_AUDIT_QUERIES, MOCK_COMPANY_MEMBERS } from "@/lib/mock/audit-data";
...
export default async function AuditorPage({ params }: { params: Promise<{ companyId: string }> }) {
  ...
  const [areas, risks, team, findings, workingPapers, queries, manualJournalHeaders, executiveIntelligence, companyMembers] = previewMode
    ? [
        MOCK_AUDIT_AREAS,
        MOCK_AUDIT_RISK_REGISTER,
        MOCK_AUDIT_TEAM_ASSIGNMENTS,
        MOCK_AUDIT_FINDINGS,
        MOCK_AUDIT_WORKING_PAPERS,
        MOCK_AUDIT_QUERIES,
        MOCK_JOURNALS.filter((j) => j.sourceType === "manual").map((j) => ({ id: j.id, journalNumber: j.journalNumber, sourceType: j.sourceType, reference: j.reference })),
        null,
  ...
  const missingDocumentsCount = answerMissingSupportingDocuments(manualJournalHeaders).supportingTransactions.length;
