...
import {
  ...
  listInvoicesForBillingAccount, listInvoiceLines, listPaymentsForBillingAccount, listCreditsForBillingAccount, listBillingEvents, listStatusHistory,
...
export default async function BillingPage({ params }: { params: Promise<{ companyId: string }> }) {
  ...
  // Finding #126 (RC-16/E13) — pre-fetched once here (one company's own
  // invoice count is small) rather than fetched on demand client-side,
  // matching this app's dominant SSR-props pattern.
  const invoiceLinesById = Object.fromEntries(
    await Promise.all(invoices.map(async (inv) => [inv.id, await listInvoiceLines(inv.id)] as const)),
  );
  ...
  return (
    <div className="flex w-full flex-col gap-6">
      ...
      <BillingTabs
        ...
        invoices={invoices}
        invoiceLinesById={invoiceLinesById}
        payments={payments}
        credits={credits}
        events={events}
