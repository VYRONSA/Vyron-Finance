...
import { ReactivateAccountButton } from "@/components/financial/reactivate-account-button";
...
export default async function BankAccountDetailPage({
  ...
  return (
    <div className="flex w-full flex-col gap-6">
      ...
      {/* Finding #079 — a statement file can carry rows for several
          accounts, so import batches aren't scoped to a single bank
          account; company-wide is the real, honest scope here. Made the
          title and every piece of copy on this card agree on that,
          rather than leaving the empty-state text implying otherwise. */}
      <Card tone="dark">
        <CardHeader>
          <CardTitle className="text-vf-on-dark">Recent Bank Statement Imports — All Accounts</CardTitle>
          <CardDescription className="text-vf-on-dark-faint">The most recent Standard VYRON Bank Import Template files imported for this company (not filtered to this account).</CardDescription>
        </CardHeader>
        <CardContent className="pt-0">
          {recentImports.length === 0 ? (
            <EmptyState
              ...
              description="Import a Standard VYRON bank statement CSV from the Import Centre to bring transactions into your accounts."
              action={
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-3">
          ...
          {account.status === "Archived" ? (
            <ReactivateAccountButton companyId={companyId} accountId={account.id} previewMode={previewMode} />
          ) : (
