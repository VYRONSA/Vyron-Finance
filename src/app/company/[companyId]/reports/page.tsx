...
import { getIncomeStatement, getBalanceSheet, getCashFlowStatement, identifyCashAccountIds } from "@/server/services/financial-statements-service";
import {
  getCashflowForecast,
...
import { MOCK_BRANCHES, MOCK_COMPANIES_FULL, MOCK_COST_CENTRES, MOCK_DEPARTMENTS, MOCK_FINANCIAL_YEARS, MOCK_PROJECTS } from "@/lib/mock/company-management-data";
import {
  MOCK_BALANCE_SHEET,
  MOCK_BUDGETS,
...
export default async function ReportsPage({ params }: { params: Promise<{ companyId: string }> }) {
  ...
  const [accounts, incomeStatement, balanceSheet, cashFlowStatement, forecastResults, budgets, reportDefinitions, executiveAlerts, executiveIntelligence, branches, departments, costCentres, projects] = previewMode
    ? [
  ...
  const cashAccountIds = new Set(identifyCashAccountIds(accounts));
  const cashAccounts = accounts.filter((a) => cashAccountIds.has(a.id));
