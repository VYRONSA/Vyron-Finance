...
import { COMMUNICATIONS_LIST_CAP, listCommunications, listTemplates } from "@/server/services/communication-service";
...
export default async function CommunicationsPage({ params }: { params: Promise<{ companyId: string }> }) {
  ...
  // Finding #063 (RC-6) — real "showing the most recent N" disclosure
  // when the Communications Log's cap is actually hit, mirroring the
  // same `entriesCapped` pattern Cashbook already established.
  const communicationsCapped = !previewMode && communications.length >= COMMUNICATIONS_LIST_CAP;
  ...
  return (
    <div className="flex w-full flex-col gap-6">
      ...
      <CommunicationsTabs
        ...
        templates={templates}
        dashboard={dashboard}
        communicationsCapped={communicationsCapped}
