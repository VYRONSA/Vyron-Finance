...
import { CASHBOOK_TRANSACTIONS_LIST_CAP, listCashbookBatches, listCashbookTransactions } from "@/server/services/cashbook-service";
...
export default async function CashbookPage({ params }: { params: Promise<{ companyId: string }> }) {
  ...
  const [bankAccountSummaries, entries, batches, reconciliations, chartOfAccounts] = previewMode
    ? [MOCK_BANK_ACCOUNT_SUMMARIES, MOCK_CASHBOOK_TRANSACTIONS, MOCK_CASHBOOK_BATCHES, MOCK_BANK_RECONCILIATIONS, MOCK_CHART_OF_ACCOUNTS]
    : await Promise.all([listBankAccountSummaries(companyId), listCashbookTransactions(companyId), listCashbookBatches(companyId), listReconciliations(companyId), listChartOfAccounts(companyId)]);

  const bankAccounts = bankAccountSummaries.map((s) => ({ id: s.account.id, accountName: s.account.accountName, currency: s.account.currency }));
  ...
  const entriesCapped = !previewMode && entries.length >= CASHBOOK_TRANSACTIONS_LIST_CAP;
