...
export default async function MatchingPage({ params }: { params: Promise<{ companyId: string }> }) {
  ...
  const [{ items: queue, hasMore: queueHasMore }, bankingAggregate, importBatches, bankingExceptions, ruleApplicationsToday, merchants, customerMatching, supplierMatching, chartOfAccounts] = previewMode
    ? [{ items: MOCK_MATCHING_QUEUE, hasMore: false }, MOCK_BANKING_AGGREGATE, [], MOCK_BANKING_EXCEPTIONS, [], MOCK_MERCHANTS, MOCK_CUSTOMER_MATCHING, MOCK_SUPPLIER_MATCHING, MOCK_CHART_OF_ACCOUNTS]
    : await Promise.all([
