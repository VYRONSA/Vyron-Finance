...
export default async function SalesPage({ params }: { params: Promise<{ companyId: string }> }) {
  ...
  const [customers, vatTreatments, quotations, orders, deliveries, invoices, receipts, stockItems, bankAccounts, chartOfAccounts] = previewMode
    ? [MOCK_CUSTOMERS, MOCK_VAT_TREATMENTS, MOCK_QUOTATIONS, MOCK_SALES_ORDERS, MOCK_DELIVERIES, MOCK_SALES_INVOICES, MOCK_CUSTOMER_RECEIPTS, MOCK_STOCK_ITEMS, [], MOCK_CHART_OF_ACCOUNTS]
    : await Promise.all([
