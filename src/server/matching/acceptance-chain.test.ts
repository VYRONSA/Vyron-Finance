...
function bill(overrides: Partial<ImportedBill> = {}): ImportedBill {
  return {
    ...
    vat: 130.43, total: 1000, outstanding: 1000, currency: "ZAR", status: "Posted", glAccount: "6100", vatCode: "STD",
...
describe("Acceptance chain — Import Bank Statement -> ... -> Audit Trail", () => {
  ...
  it("Stage 10: Bank Reconciliation — real outstanding items and a real bank-vs-GL difference are computed", () => {
    ...
    // Genuinely posted to the General Ledger (`postedFlag`, set by the
    // posting engine alongside the gl_transactions rows) but not yet
    // reconciled this session — outstanding, and honestly auto-matchable
    // because the ledger entry ticking it off would assert really exists.
    const postedNotReconciled = txn({ id: 101, journalId: 500, postedFlag: true, reconciliationId: null, debit: 1000, credit: 0, transactionDate: "2026-06-05" });
    ...
    // The two unreconciled payments (1000 + 200) are what stands between
    // the opening balance and the statement's closing balance, and the
    // engine says so in money terms rather than only reporting a flag.
    expect(summary.differenceReasons.find((r) => r.label.startsWith("Payments"))).toMatchObject({ amount: -1200, count: 2 });
  });
