...
describe("buildIncomeStatement", () => {
  ...
  // Phase 30C — was `.localeCompare()` (lexicographic); GL account codes
  // must sort numerically ascending in every reporting engine. Uses a
  // 5-digit code deliberately, the one case where lexicographic and
  // numeric ordering genuinely disagree: "10000" < "6100" lexicographically
  // (since '1' < '6'), but 10000 > 6100 numerically.
  it("sorts Operating Expenses lines numerically ascending, not lexicographically", () => {
    const mixedExpenseAccounts = [
      ...ACCOUNTS,
      account(101, "6940", "Salaries & Wages", "Expense", "Debit"),
      account(102, "6300", "Inventory Adjustments", "Expense", "Debit"),
      account(103, "10000", "New Overflow Expense", "Expense", "Debit"),
    ];
    const extraRows = [
      row(101, "6940", "Expense", "Debit", 1000, 0),
      row(102, "6300", "Expense", "Debit", 150, 0),
      row(103, "10000", "Expense", "Debit", 300, 0),
    ];
    const startExtras = extraRows.map((r) => ({ ...r, totalDebit: 0, totalCredit: 0 }));
    const statement = buildIncomeStatement(mixedExpenseAccounts, [...START_ROWS, ...startExtras], [...END_ROWS, ...extraRows], "2026-01-01", "2026-01-31");
    const expenseCodesFromExtras = statement.operatingExpenses.lines.map((l) => l.accountCode).filter((c) => ["6300", "6940", "10000"].includes(c));
    // Numeric truth: 6300 < 6940 < 10000. A lexicographic sort would have
    // produced ["10000", "6300", "6940"] instead — genuinely wrong.
    expect(expenseCodesFromExtras).toEqual(["6300", "6940", "10000"]);
  });
});
