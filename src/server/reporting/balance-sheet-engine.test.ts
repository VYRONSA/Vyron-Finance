...
describe("buildBalanceSheet", () => {
  ...
  // must sort numerically ascending in every reporting engine.
  it("sorts Asset section lines numerically ascending, not lexicographically", () => {
    const mixedAccounts: ChartOfAccount[] = [
      account(1, "1200", "Asset", "Debit"),
      account(2, "500", "Asset", "Debit"),
      account(3, "1000", "Asset", "Debit"),
    ];
    const rows: TrialBalanceRow[] = [
      row(1, "Asset", "Debit", 1000, 0),
      row(2, "Asset", "Debit", 500, 0),
      row(3, "Asset", "Debit", 2000, 0),
    ];
    const sheet = buildBalanceSheet(mixedAccounts, rows, "2026-01-31", 0);
    // Numerically: 500 < 1000 < 1200. A lexicographic sort would have
    // produced ["1000", "1200", "500"] instead — genuinely wrong.
    expect(sheet.assets.lines.map((l) => l.accountCode)).toEqual(["500", "1000", "1200"]);
  });
});
