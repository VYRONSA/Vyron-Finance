...
import { buildVatIntelligence, detectMissingVatNumber, detectVatRateConflict, isHighRisk } from "@/server/vat/vat-intelligence";
...
/** Phase 25C — the exact same "high-risk VAT document" definition
 * `app/company/[companyId]/vat/page.tsx` and
 * `vat-exception-scan-service.ts::runVatIntelligenceScan` already use: a
 * document is high-risk once `isHighRisk` says so (2+ independent VAT
 * Intelligence signals, or one signal at 90+ confidence) over the real,
 * already-computed `buildVatIntelligence`/`detectMissingVatNumber`
 * signals — no second VAT detector, no competing definition. Returns a
 * genuine count of distinct high-risk documents, matching the field's own
 * name (`highRiskVatTransactionCount`), not a boolean/bucket.
 *
 * Phase 25K — `buildVatIntelligence` only runs `detectSuspiciousVatValues`
 * (flat "compare against today's rate"), never the date-aware
 * `detectVatRateConflict` that `vat-exception-scan-service.ts` already
 * wires in alongside it. Without it, this count could both false-positive
 * on legitimate historical documents dated before a real rate change, and
 * miss genuine same-treatment rate conflicts. Mirrors that file's own
 * `historyByTreatmentCode` construction exactly — same source query
 * (`listRateHistoryForCompany`), just re-keyed by code — no new detector,
 * no second VAT engine. */
async function countHighRiskVatTransactions(companyId: string): Promise<number> {
  ...
  const rateConflictMap = detectVatRateConflict(documents, historyByTreatmentCode);

  return [...new Set([...intelligenceMap.keys(), ...missingVatNumberMap.keys(), ...rateConflictMap.keys()])].filter((id) => {
    const signals = [
      ...(intelligenceMap.get(id) ?? []),
      ...(missingVatNumberMap.get(id) ? [missingVatNumberMap.get(id)!] : []),
      ...(rateConflictMap.get(id) ? [rateConflictMap.get(id)!] : []),
    ];
...
export async function getExecutiveIntelligence(companyId: string, dateFrom: string, dateTo: string, financialYearStartDate: string): Promise<ExecutiveIntelligenceReport> {
  const [financialIntelligence, openBankingExceptions, openVatExceptions, vatReturns, balanceSheet, cashflowForecast, incomeStatement, salesInvoices, supplierRisk, highRiskVatTransactionCount, automationTasks] = await Promise.all([
    ...
    countHighRiskVatTransactions(companyId),
  ...
  // Same overdue predicate `customer-financial-service.ts::getCustomerFinancialSummary`
  // already uses per-customer (outstanding > 0, has a due date, past it),
  // applied here company-wide; Credit Notes excluded to match
  // `company-intelligence-service.ts`'s debtors-aging convention.
  const referenceDateMs = Date.parse(dateTo);
  const overdueDebtorsCount = salesInvoices.filter(
    (i) => i.documentType !== "Credit Note" && i.outstanding > 0 && i.dueDate !== null && Date.parse(i.dueDate) < referenceDateMs,
  ).length;
  ...
  const businessRiskScore = computeBusinessRiskScore({
    ...
    highRiskVatTransactionCount,
    overdueDebtorsCount,
    // Reuses detectSupplierRisk's own concentration-share check (same
    // function raiseAlert's SupplierRisk candidate comes from) rather than
    // re-deriving the share — a binary risk flag, not a supplier count.
    supplierConcentrationRiskCount: supplierRisk !== null ? 1 : 0,
  ...
  const auditReadinessScore = computeAuditReadinessScore({
    ...
    openBankingExceptionCount: openBankingExceptions.length,
    openVatExceptionCount: openVatExceptions.length,
    // Same real source detectAutomationFailures already uses for its own
    // Executive Alert — reused here rather than a second failed-task
    // query, matching the overdueDebtorsCount/supplierConcentrationRiskCount/
    // highRiskVatTransactionCount fixes already applied to this function.
    failedAutomationTaskCount: automationTasks.filter((t) => t.status === "Failed").length,
  });
