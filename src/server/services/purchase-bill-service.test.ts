...
import { computeBillableLines, computeBillLine, lineAmountFor, ValidationError } from "./purchase-bill-service";
...
describe("computeBillableLines", () => {
  it("Finding #056 — bills only the received-but-not-yet-billed remainder of a PartiallyReceived order", () => {
    const result = computeBillableLines([
      { id: 1, quantity: 10, receivedQuantity: 6, billedQuantity: 0 },
      { id: 2, quantity: 5, receivedQuantity: 5, billedQuantity: 5 },
    ]);
    expect(result).toEqual([{ id: 1, quantity: 10, receivedQuantity: 6, billedQuantity: 0, billQuantity: 6 }]);
  });

  it("excludes a line with nothing new to bill", () => {
    expect(computeBillableLines([{ id: 1, quantity: 10, receivedQuantity: 6, billedQuantity: 6 }])).toEqual([]);
  });
});
