...
/** Finding #096 — Resolved/Dismissed was a one-way door; this is the
 * explicit reverse action, mirroring `resolveException`'s own
 * status-guard shape. */
export async function reopenException(companyId: string, exceptionId: number): Promise<BankingException> {
  const existing = await repo.getBankingException(companyId, exceptionId);
  if (!existing) throw new NotFoundError(`No exception with id ${exceptionId}.`);
  if (existing.status === "Open") throw new ValidationError("Exception is already Open.");
  return repo.reopenException(companyId, exceptionId);
}
