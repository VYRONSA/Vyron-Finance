...
export async function postApprovedJournals(companyId: string): Promise<PostApprovedJournalsOutcome> {
  ...
  // #041. `postApprovedJournalsAtomic` claims-and-writes in one DB
  // statement (0063_atomic_journal_posting.sql) — a concurrent posting
  // run can never double-claim a journal this call already claimed, and
  // the batch/GL-transaction/journal-status writes can never partially
  // succeed. `claimedJournalIds` is authoritative: if a journal this call
  // validated got claimed by a concurrent run first, it's excluded here
  // rather than reported as posted.
  const performedBy = await getPerformedByLabel();
  const batchNumber = await postingRepo.nextPostingBatchNumber(companyId);
  const postingDate = new Date().toISOString().slice(0, 10);
  const { batch, claimedJournalIds } = await postingRepo.postApprovedJournalsAtomic(
    companyId,
    posted.map((p) => p.journalId),
    rows.map((r) => ({ ...r, postedBy: performedBy })),
    batchNumber,
    postingDate,
    performedBy,
  );

  const claimedIdSet = new Set(claimedJournalIds);
  const actuallyPosted = posted.filter((p) => claimedIdSet.has(p.journalId));
  const lostToConcurrentRun = posted
    .filter((p) => !claimedIdSet.has(p.journalId))
    .map((p) => ({ journalId: p.journalId, journalNumber: p.journalNumber, reason: "Already posted by another concurrent posting run." }));

  return { batch, posted: actuallyPosted, skipped: [...skipped, ...lostToConcurrentRun] };
}
