...
import type { SupplierReconciliationScope } from "@/server/repositories/supplier-reconciliation-repository";
 ...
 */
export async function generateSupplierAllocationReports(companyId: string, scope: SupplierReconciliationScope = {}): Promise<GenerateReportsResult> {
  const stageResults: PipelineStageResult[] = [];

  const matching = await runStage("matching", () => runMatchingForCompany(companyId, scope));
  stageResults.push(matching);
  if (!matching.succeeded) return { stageResults, failedStage: "matching" };

  const allocation = await runStage("allocation", () => runAllocationForCompany(companyId, scope));
  stageResults.push(allocation);
  if (!allocation.succeeded) return { stageResults, failedStage: "allocation" };

  const workQueue = await runStage("work-queue", () => syncWorkQueue(companyId, scope));
 ...
 */
export async function runMatchingForCompany(companyId: string, scope: SupplierReconciliationScope = {}): Promise<number> {
  const [suppliers, bills, transactions] = await Promise.all([
    repo.listSuppliers(companyId),
    repo.listOpenBills(companyId, scope),
    repo.listBankTransactions(companyId, scope),
  ]);
 ...
 */
export async function runAllocationForCompany(companyId: string, scope: SupplierReconciliationScope = {}): Promise<number> {
  const [suppliers, bills, transactions] = await Promise.all([
    repo.listSuppliers(companyId),
    repo.listOpenBills(companyId, scope),
    repo.listBankTransactions(companyId, scope),
 ...
 */
export async function syncWorkQueue(companyId: string, scope: SupplierReconciliationScope = {}): Promise<number> {
  const supabase = await createClient();
  const transactions = await repo.listBankTransactions(companyId, scope);
