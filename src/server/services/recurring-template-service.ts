...
export async function generateFromTemplate(companyId: string, template: RecurringTemplate, todayIso: string, performedBy = "System"): Promise<GenerationOutcome> {
  ...
  // Crash-recovery guard (Phase 25K) — see `findGeneratedDocumentSince`.
  // A dangling Success row from an interrupted prior run means the
  // document was already generated; only finish advancing the template
  // instead of dispatching (and posting) a duplicate.
  const dangling = await repo.findGeneratedDocumentSince(companyId, template.id, template.lastRunDate);
  if (dangling) {
    ...
    return { status: "Success", document: dangling };
  }
...
async function dispatchGeneration(companyId: string, template: RecurringTemplate, todayIso: string, performedBy: string): Promise<DispatchOutcome> {
  ...
  switch (template.documentType) {
    ...
    case "SupplierBill": {
      /** Finding #205 — the payload's `lines` (per-line GL/VAT/discount,
       * same shape Purchase Bills already support via `BillLineInput`)
       * used to be silently discarded — only the legacy single-subtotal
       * fields were ever read, mirroring `CustomerInvoice`'s own `lines`
       * pass-through immediately above. */
      const p = payload as {
        supplierId: number;
        ...
        lines?: { description: string; glAccount: string; vatCode: string; costCentreId?: number | null; projectId?: number | null; departmentId?: number | null; quantity: number; unitCost: number; discount?: number }[];
      };
...
export async function requestActivationApproval(companyId: string, templateId: number, performedBy: string) {
  ...
  const instance = await workflowService.startWorkflow(companyId, template.workflowDefinitionId, "RecurringTemplate", template.id, performedBy);
  await notificationService.createNotification(companyId, {
