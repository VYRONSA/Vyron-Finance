 ...
 * (e.g. a Recurring Template) at step 0, `Pending`. `requestedBy` is
 * recorded so `decideStep` can stop the same person who requested this
 * approval from also deciding it — see Finding #125. */
export async function startWorkflow(companyId: string, definitionId: number, subjectType: string, subjectId: number, requestedBy: string): Promise<WorkflowInstance> {
  ...
  return repo.createWorkflowInstance(companyId, definitionId, subjectType, subjectId, requestedBy);
}
...
export async function decideStep(companyId: string, instanceId: number, decision: "Approved" | "Rejected", decidedBy: string, note = ""): Promise<WorkflowInstance> {
  ...
  // Finding #125 — the same person who requested this approval cannot
  // also decide it themselves, whichever module the subject belongs to.
  if (instance.requestedBy === decidedBy) throw new ValidationError("You cannot approve or reject your own request.");
