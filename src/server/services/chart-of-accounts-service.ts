...
import { compareGlAccountCodes, type AccountType, type ChartOfAccount, type ChartOfAccountNode, type NormalBalance } from "@/server/general-ledger/types";
 ...
 * hides an account from the tree view. Children are sorted NUMERICALLY by
 * account code (Phase 30 — was `.localeCompare()`, lexicographic) at every
 * level, independent of input order. */
export function buildAccountTree(accounts: ChartOfAccount[]): ChartOfAccountNode[] {
  ...
  const sortRecursive = (nodes: ChartOfAccountNode[]) => {
    nodes.sort((a, b) => compareGlAccountCodes(a.accountCode, b.accountCode));
...
function isDuplicateAccountCode(error: unknown): boolean {
  if (!error || typeof error !== "object") return false;
  const code = (error as { code?: string }).code;
  const message = (error as { message?: string }).message ?? "";
  return code === "23505" && message.includes("chart_of_accounts");
}
...
export async function createChartOfAccount(companyId: string, input: repo.NewChartOfAccount): Promise<ChartOfAccount> {
  ...
  try {
    return await repo.createChartOfAccount(companyId, { ...input, accountCode: input.accountCode.trim(), description: input.description.trim() });
  } catch (error) {
    if (isDuplicateAccountCode(error)) {
      throw new ValidationError(`Account code "${input.accountCode.trim()}" already exists.`);
    }
    throw error;
  }
}
...
/** Phase 32 — the "Download Template" button's header source. Deliberately
 * NOT `CSV_HEADERS` above: that array is the EXPORT contract (includes
 * `Active`, which `parseChartOfAccountsCsv` below never reads) — this is
 * scoped to exactly the columns the IMPORTER recognises, so the template
 * never implies a column does something it doesn't. */
export const CHART_OF_ACCOUNTS_IMPORT_TEMPLATE_HEADERS = [
  "Account Code",
  "Description",
  "Account Type",
  "Normal Balance",
...
export function parseChartOfAccountsCsv(csvText: string): { rows: ChartOfAccountImportRow[]; errors: string[] } {
  ...
  if (idxCode === undefined || idxDescription === undefined || idxType === undefined || idxBalance === undefined) {
    return {
      rows: [],
      errors: [
        `Missing required columns: Account Code, Description, Account Type, Normal Balance (headers found: ${headerRow.map(cleanText).join(", ") || "(none)"}). Download the Chart of Accounts Import Template and use its column headers.`,
      ],
    };
  }
...
export async function importChartOfAccountsCsv(companyId: string, csvText: string): Promise<{ created: ChartOfAccount[]; errors: string[] }> {
  ...
  for (const row of rows) {
    ...
    if (!childId) continue; // this row's own account failed to create; already reported above

    // Phase 32A — a non-blank Parent Account that doesn't resolve to a real
    // account code (typo, or a code absent from both the file and the
    // company) must be reported, not silently dropped as if no parent had
    // been specified at all. The child account itself was already created
    // successfully in the loop above, so this only reports the link failure.
    if (row.parentAccountCode === row.accountCode) {
      errors.push(`Account Code "${row.accountCode}": Parent Account cannot be the account's own code — parent link skipped.`);
      continue;
    }
    const parentId = codeToId.get(row.parentAccountCode);
    if (!parentId) {
      errors.push(`Account Code "${row.accountCode}": Parent Account "${row.parentAccountCode}" was not found — parent link skipped.`);
      continue;
    }
