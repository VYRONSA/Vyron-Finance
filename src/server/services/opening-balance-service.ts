...
// Master Implementation Tracker — Epic E1, Root Cause RC-1, Finding
// #184. `postOpeningBalances`'s `createJournal` call used to omit
// `journalDate` entirely, silently defaulting to today — wrong for
// opening balances, which are routinely captured weeks after the date
// they're actually effective as at. Every draft entry being posted must
// agree on that date (the grid now sends one shared `balanceDate` for a
// whole save — see `gl-opening-balances-grid.tsx`); a real mismatch is
// surfaced rather than silently resolved by picking one, matching this
// module's own "refuses to post an unbalanced set" discipline. Pure and
// exported for the same reason `netOpeningBalanceLines` is — directly
// unit-testable without a live database.
export function resolveOpeningBalanceJournalDate(entries: { balanceDate: string }[]): string {
  if (entries.length === 0) throw new ValidationError("No draft entries to derive a balance date from.");
  const dates = new Set(entries.map((e) => e.balanceDate));
  if (dates.size > 1) {
    throw new ValidationError(
      `The draft entries being posted don't all share the same balance date (${[...dates].sort().join(", ")}). Re-save the grid with one balance date before posting.`,
    );
  }
  return entries[0].balanceDate;
}
...
export async function postOpeningBalances(companyId: string, performedBy: string): Promise<PostOpeningBalancesResult> {
  ...
  const journalDate = resolveOpeningBalanceJournalDate(draftEntries);
  ...
  // #185. `postApprovedJournals`'s outcome used to be discarded here —
  // every draft entry got marked "posted" (permanently locking them from
  // further edits) and every linked bank account's cached balance got
  // synced regardless of whether the Posting Engine actually posted the
  // journal. A journal is legitimately skipped when its date falls in a
  // closed/locked financial period; that must stop this function here,
  // not leave the books silently out of balance with entries the UI
  // claims are posted but that never reached `gl_transactions`.
  ...
  if (!wasPosted) {
    ...
    throw new ValidationError(
      `Opening balances journal ${journal.journalNumber} could not be posted to the General Ledger${skip ? `: ${skip.reason}` : "."} No entries were marked as posted — correct the issue and try again.`,
    );
  }
