...
import { computeOpeningBalanceDelta, maskAccountNumber, validateBankAccountGlCode, validateCreateBankAccountInput, ValidationError } from "./bank-account-service";
...
// Findings #019/#077/#220. Only the blank/undefined case resolves
// synchronously before any Supabase call — a real code check needs the
// Chart of Accounts, matching this file's own established convention of
// testing the pure/no-IO paths of this service directly.
describe("validateBankAccountGlCode", () => {
  it("resolves without an existence check when blank", async () => {
    await expect(validateBankAccountGlCode("company-1", "")).resolves.toBeUndefined();
    await expect(validateBankAccountGlCode("company-1", undefined)).resolves.toBeUndefined();
    await expect(validateBankAccountGlCode("company-1", "   ")).resolves.toBeUndefined();
  });
});
