...
export type SupplierMatchingWorkspaceData = {
  ...
  // Finding #148 — see the identical note in customer-matching-service.ts.
  suppliers: { id: number; name: string }[];
};

export async function getSupplierMatchingWorkspace(companyId: string): Promise<SupplierMatchingWorkspaceData> {
  const [allBills, payments, suppliers] = await Promise.all([listAllBills(companyId), listSupplierPayments(companyId), listSuppliers(companyId)]);
  return {
