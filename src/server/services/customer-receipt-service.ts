...
/** Finding #031 — the reverse of `allocateReceipt`: removes one
 * allocation line and restores the amount onto the invoice's
 * outstanding balance. Deliberately does not require the receipt to
 * still be `Posted` — a receipt can only reach Posted before it's ever
 * allocated, and undoing a misapplied allocation must stay possible
 * regardless of the receipt's later state. */
export async function unallocateReceipt(companyId: string, allocationId: number, performedBy: string): Promise<void> {
  const deleted = await repo.deleteReceiptAllocation(companyId, allocationId);
  if (!deleted) throw new NotFoundError(`No allocation with id ${allocationId}.`);

  await invoiceRepo.reduceInvoiceOutstanding(companyId, deleted.invoiceId, -deleted.amountAllocated);
  await recordOverride(companyId, {
    itemType: "CustomerReceiptAllocation",
    itemId: allocationId,
    fieldName: "invoiceId",
    oldValue: String(deleted.invoiceId),
    newValue: null,
    reason: `Unallocated ${deleted.amountAllocated} from invoice ${deleted.invoiceId}`,
    performedBy,
  });
}
