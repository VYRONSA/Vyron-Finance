...
export type MatchingQueueResult = {
  items: MatchingQueueItem[];
  /** True when more unmatched bank transactions exist than
   * `QUEUE_PAGE_SIZE` — see the Finding #029 disclosure below. */
  hasMore: boolean;
};

export async function getMatchingQueue(companyId: string): Promise<MatchingQueueResult> {
  ...
  // Finding #029 (RC-6): disclosed to the caller via `hasMore` below —
  // ReviewQueueTab renders a "showing the most recent N — refine filters
  // to see more" banner instead of silently truncating with no signal.
  ...
  return { items, hasMore: unmatchedResult.hasMore };
}
