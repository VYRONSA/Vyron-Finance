...
export const listManualJournalHeaders = journalRepo.listManualJournalHeaders;
...
export async function reverseJournal(companyId: string, journalId: number): Promise<ReverseJournalOutcome> {
  ...
  // Phase 25K — a double-click/double-tab on "Reverse" (or any other
  // retry of this same request) could otherwise race between this read
  // and the `markJournalReversed` stamp below, creating two independent
  // reversal journals for one original before `original.isReversed`
  // above ever sees the first one land. Mirrors the exact
  // `getJournalBySource` dedup pattern already established for the Rule
  // Engine's own journal-posting path (`rule-processing-service.ts`) —
  // same narrow, accepted residual window (a genuinely simultaneous
  // request landing between this check and `createJournal` below),
  // same fix.
  const existingReversal = await journalRepo.getJournalBySource(companyId, "journal_reversal", original.id);
  if (existingReversal) {
    const refreshedOriginal = await requireJournal(companyId, original.id);
    return { original: refreshedOriginal, reversal: existingReversal };
  }
