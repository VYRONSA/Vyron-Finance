...
import { autoMatchableTransactionIds, buildReconciliationSummary, reconciliationReadiness, type ReconciliationSummary } from "@/server/banking/reconciliation-engine";
...
export type StartReconciliationInput = {
  ...
  /** The statement period: start and end. `statementDate` remains the
   * END date — the meaning it has always had on this table — so nothing
...
export async function startReconciliation(companyId: string, input: StartReconciliationInput): Promise<BankReconciliation> {
  const { bankAccountId, statementDate, statementClosingBalance } = input;
  ...
  const periodStart = input.statementPeriodStart?.trim() || null;
  if (periodStart && periodStart > statementDate) {
    throw new ValidationError("The statement period cannot start after it ends.");
  }

  const glClosingBalance = await computeGlClosingBalance(companyId, bankAccountId, statementDate);
  const reconciliation = await repo.createReconciliation(companyId, {
    bankAccountId,
    statementDate,
    statementPeriodStart: periodStart,
    statementOpeningBalance: round2(input.statementOpeningBalance ?? 0),
...
export async function getReconciliationSummary(companyId: string, reconciliationId: number): Promise<ReconciliationSummary> {
  ...
  // The GL balance is recomputed live rather than read off the stored
  // column: posting transactions from the Explorer changes it, and a
  // reconciliation opened before that posting would otherwise keep
  // showing a stale figure and an imaginary difference.
  ...
  return buildReconciliationSummary(transactions, reconciliation.statementDate, reconciliation.statementClosingBalance, glClosingBalance, {
    periodStart: reconciliation.statementPeriodStart,
    statementOpeningBalance: reconciliation.statementOpeningBalance,
    reconciliationId: reconciliation.id,
  });
}

/** The plain-language "why doesn't this balance?" list the workspace
 * shows above the difference. Derived, never stored. */
export async function getReconciliationReadiness(companyId: string, reconciliationId: number): Promise<string[]> {
  return reconciliationReadiness(await getReconciliationSummary(companyId, reconciliationId));
}

/** Confirming or correcting the statement's own opening/closing figures
 * mid-reconciliation — the accountant reads them off the paper statement
 * and they are frequently keyed in wrong the first time. Blocked once the
 * reconciliation is Completed; the recorded difference is recomputed so
 * the stored figure never lags what the workspace shows. */
export async function updateStatementFigures(
  companyId: string,
  reconciliationId: number,
  figures: { statementPeriodStart?: string | null; statementOpeningBalance?: number; statementClosingBalance?: number },
  ...
  const closing = figures.statementClosingBalance ?? reconciliation.statementClosingBalance;
  const periodStart = figures.statementPeriodStart === undefined ? reconciliation.statementPeriodStart : (figures.statementPeriodStart?.trim() || null);
  if (periodStart && periodStart > reconciliation.statementDate) {
    throw new ValidationError("The statement period cannot start after it ends.");
  }
  ...
  return repo.updateReconciliation(companyId, reconciliationId, {
    ...
    statement_opening_balance: round2(figures.statementOpeningBalance ?? reconciliation.statementOpeningBalance),
    statement_closing_balance: round2(closing),
    gl_closing_balance: glClosingBalance,
    difference: round2(closing - glClosingBalance),
  });
}
