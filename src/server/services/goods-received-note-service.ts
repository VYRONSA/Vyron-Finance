...
export async function cancelGoodsReceivedNote(companyId: string, grnId: number): Promise<GoodsReceivedNote> {
  ...
  /** Finding #189 — a GRN that already has a real, non-cancelled Bill
   * against it (a GRNI-clearing entry may already be posted) can't be
   * silently cancelled out from under that bill. */
  const bills = await listPurchaseBills(companyId);
  const activeBill = bills.find((b) => b.goodsReceivedNoteId === grnId && b.postingStatus !== "Cancelled");
  if (activeBill) {
    throw new ValidationError(`Cannot cancel ${grn.grnNumber} — ${activeBill.invoiceNumber} has already been billed against it.`);
  }
  ...
  for (const line of grn.lines) {
    if (line.orderLineId) await incrementOrderLineQuantity(companyId, line.orderLineId, "received_quantity", -line.quantity);
  }
