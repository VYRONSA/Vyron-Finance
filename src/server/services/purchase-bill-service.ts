...
import { getGoodsReceivedNote, listGoodsReceivedNotes } from "@/server/repositories/goods-received-note-repository";
...
/** Finding #056 — mirrors `sales-invoice-service.ts::computeInvoiceableLines`
 * exactly. Pure — unit tested. */
export function computeBillableLines<T extends { quantity: number; receivedQuantity: number; billedQuantity: number }>(lines: T[]): (T & { billQuantity: number })[] {
  return lines
    .map((l) => ({ ...l, billQuantity: Math.max(0, l.receivedQuantity - l.billedQuantity) }))
...
export async function createBillFromOrder(
  ...
  if (order.status !== "Received" && order.status !== "PartiallyReceived") {
    throw new ValidationError(`Only a Received or PartiallyReceived order can be billed (current status: ${order.status}).`);
  }

  const billableLines = computeBillableLines(order.lines);
  if (billableLines.length === 0) {
    throw new ValidationError(`${order.orderNumber} has nothing received that hasn't already been billed.`);
  }

  const everyLineDimensioned = billableLines.every((l) => l.glAccount && l.vatCode);

  /** Finding #221 — `goodsReceivedNoteId` was never set on a Bill created
   * from an order, even though the column exists and is real. An order
   * can be received across multiple partial GRNs, so there isn't always
   * one unambiguous GRN to attribute the Bill to — only set it when
   * exactly one non-cancelled GRN exists against this order. */
  const grns = (await listGoodsReceivedNotes(companyId)).filter((g) => g.orderId === orderId && g.status !== "Cancelled");
  const goodsReceivedNoteId = grns.length === 1 ? grns[0].id : null;
  ...
  const bill = await createPurchaseBill(
    ...
    everyLineDimensioned
      ...
      : {
          ...
          invoiceDate,
          vatTreatmentCode,
          subtotal: round2(billableLines.reduce((sum, l) => sum + l.billQuantity * l.unitPrice, 0)),
          purchaseOrderId: order.id,
          goodsReceivedNoteId,
        },
  );
  ...
  for (const line of billableLines) {
    await orderRepo.incrementOrderLineQuantity(companyId, line.id, "billed_quantity", line.billQuantity);
  }

  const fullyBilled = order.lines.every((l) => {
    const billed = billableLines.find((bl) => bl.id === l.id)?.billQuantity ?? 0;
    return l.billedQuantity + billed >= l.quantity;
  });
  if (fullyBilled) await orderRepo.setOrderStatus(companyId, orderId, "Billed");

  return bill;
}

/** Finding #204 — a standalone GRN (`orderId === null`, received with no
 * Purchase Order behind it) could never become a Bill at all — the only
 * from-order path was `createBillFromOrder`, which requires a real
 * PurchaseOrder. Mirrors it: GRN lines carry no per-line GL/VAT
 * dimensions (unlike Purchase Order lines), so this always uses the
 * legacy single-subtotal path, same as `createBillFromOrder` falls back
 * to whenever its order's lines aren't fully dimensioned. */
export async function createBillFromGrn(
  companyId: string,
  grnId: number,
  invoiceNumber: string,
  invoiceDate: string,
  ...
  if (grn.orderId !== null) {
    throw new ValidationError(`${grn.grnNumber} is linked to a Purchase Order — bill it from that order instead.`);
  }
  if (grn.status !== "Received") {
    throw new ValidationError(`Only a Received GRN can be billed (current status: ${grn.status}).`);
  }

  const existingBills = await listGoodsReceivedNoteBills(companyId, grnId);
  if (existingBills.length > 0) {
    throw new ValidationError(`${grn.grnNumber} has already been billed (${existingBills[0].invoiceNumber}).`);
  }

  return createPurchaseBill(companyId, {
    supplierId: grn.supplierId,
    invoiceNumber,
    invoiceDate,
    vatTreatmentCode,
    subtotal: round2(grn.lines.reduce((sum, l) => sum + l.quantity * l.unitCost, 0)),
    goodsReceivedNoteId: grn.id,
  });
}

async function listGoodsReceivedNoteBills(companyId: string, grnId: number): Promise<ImportedBill[]> {
  const bills = await repo.listPurchaseBills(companyId);
  return bills.filter((b) => b.goodsReceivedNoteId === grnId && b.postingStatus !== "Cancelled");
}
