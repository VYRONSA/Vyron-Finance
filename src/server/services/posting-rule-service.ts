 ...
 * to `CSV_HEADERS` above, kept as a distinct exported name — see the
 * same note on `banking-rule-service.ts::BANKING_RULES_IMPORT_TEMPLATE_HEADERS`. */
export const POSTING_RULES_IMPORT_TEMPLATE_HEADERS = CSV_HEADERS;
...
export function parsePostingRulesCsv(csvText: string): { rows: PostingRuleImportRow[]; errors: string[] } {
  ...
  if (idxEventType === undefined || idxSide === undefined || idxRole === undefined || idxAmountSource === undefined) {
    return {
      rows: [],
      errors: [
        `Missing required columns: Event Type, Side, Role, Amount Source (headers found: ${headerRow.map(cleanText).join(", ") || "(none)"}). Download the Posting Rules Import Template and use its column headers.`,
      ],
    };
  }
  ...
  dataRows.forEach((raw, i) => {
    ...
    // Phase 32A — a blank Line Order legitimately falls back to "next
    // position within this event type," but a non-blank cell that isn't a
    // valid number must reject the row: the old `raw ? Number(raw) : ...`
    // let garbage text through as `Number(x)`, silently storing `NaN`,
    // which then broke the numeric `.sort((a, b) => a.lineOrder - b.lineOrder)` below.
    let lineOrder: number;
    if (!lineOrderRaw.trim()) {
      lineOrder = rows.filter((r) => r.eventType === eventType).length;
    } else {
      const parsed = Number(lineOrderRaw);
      if (!Number.isFinite(parsed)) return void errors.push(`Row ${rowNumber}: Line Order "${lineOrderRaw}" is not a valid number.`);
      lineOrder = parsed;
    }
