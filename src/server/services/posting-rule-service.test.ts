...
import {
  ...
  ValidationError,
  POSTING_RULES_IMPORT_TEMPLATE_HEADERS,
...
describe("buildPostingRulesCsv / parsePostingRulesCsv", () => {
  ...
  it("the missing-columns error points the user at the Posting Rules Import Template", () => {
    const { errors } = parsePostingRulesCsv("Foo,Bar\n1,2");
    expect(errors[0]).toContain("Download the Posting Rules Import Template");
  });

  // Phase 32A — a non-blank Line Order that isn't a valid number must
  // reject the row: the old `raw ? Number(raw) : ...` let garbage text
  // through as `Number(x)`, silently storing `NaN`, which then broke the
  // numeric `.sort((a, b) => a.lineOrder - b.lineOrder)` grouping step.
  it("rejects a row with a non-numeric Line Order instead of silently storing NaN", () => {
    const csv = ["Event Type,Description,Active,Line Order,Side,Role,Fixed Account Code,Amount Source", "Sales Invoice,,Yes,not-a-number,Debit,bank,1000,gross"].join("\n");
    const { rows, errors } = parsePostingRulesCsv(csv);
    expect(rows).toEqual([]);
    expect(errors).toEqual(['Row 2: Line Order "not-a-number" is not a valid number.']);
  });

  it("defaults Line Order to the row's position within its Event Type when blank", () => {
    const csv = [
      "Event Type,Description,Active,Line Order,Side,Role,Fixed Account Code,Amount Source",
      "Sales Invoice,,Yes,,Debit,debtors,1100,gross",
      "Sales Invoice,,Yes,,Credit,sales,4000,net",
    ].join("\n");
    const { rows, errors } = parsePostingRulesCsv(csv);
    expect(errors).toEqual([]);
    expect(rows.map((r) => r.lineOrder)).toEqual([0, 1]);
  });

  it("still accepts a valid numeric Line Order", () => {
    const csv = ["Event Type,Description,Active,Line Order,Side,Role,Fixed Account Code,Amount Source", "Sales Invoice,,Yes,3,Debit,bank,1000,gross"].join("\n");
    const { rows, errors } = parsePostingRulesCsv(csv);
    expect(errors).toEqual([]);
    expect(rows[0].lineOrder).toBe(3);
  });
});

// Phase 32 — "the template must be generated from the ACTUAL importer
// contract... do not invent columns."
describe("POSTING_RULES_IMPORT_TEMPLATE_HEADERS (Phase 32)", () => {
  it("is accepted end-to-end by the parser with no missing-columns rejection", () => {
    const csv = [POSTING_RULES_IMPORT_TEMPLATE_HEADERS.join(","), "Sales Invoice,Sales,Yes,0,Debit,debtors,1100,gross"].join("\n");
    const { rows, errors } = parsePostingRulesCsv(csv);
    expect(errors).toEqual([]);
    expect(rows).toHaveLength(1);
    expect(rows[0]).toMatchObject({ eventType: "Sales Invoice", side: "Debit", role: "debtors", fixedAccountCode: "1100", amountSource: "gross" });
