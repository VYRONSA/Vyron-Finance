...
export async function generateReportingPackage(
  ...
  // Phase 25K — the package itself is already persisted by this point; a
  // transient usage-metering failure must not turn an already-successful
  // generation into a reported error (same established `.catch` guard as
  // every other `recordUsageEvent` call site, e.g. bank-sync-service.ts).
  await recordUsageEvent(companyId, "reports_generated").catch(() => {});
