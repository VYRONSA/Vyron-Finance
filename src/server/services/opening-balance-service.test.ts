...
import { netOpeningBalanceLines, resolveOpeningBalanceJournalDate, ValidationError } from "./opening-balance-service";
...
// Master Implementation Tracker — Epic E1, Root Cause RC-1, Finding #184.
describe("resolveOpeningBalanceJournalDate", () => {
  it("uses the shared balance date when every entry agrees", () => {
    expect(
      resolveOpeningBalanceJournalDate([{ balanceDate: "2026-03-01" }, { balanceDate: "2026-03-01" }, { balanceDate: "2026-03-01" }]),
    ).toBe("2026-03-01");
  });

  it("never silently defaults to today when entries disagree — throws instead", () => {
    expect(() => resolveOpeningBalanceJournalDate([{ balanceDate: "2026-03-01" }, { balanceDate: "2026-03-02" }])).toThrow(ValidationError);
  });

  it("throws rather than deriving a date from an empty entry list", () => {
    expect(() => resolveOpeningBalanceJournalDate([])).toThrow(ValidationError);
  });
});
