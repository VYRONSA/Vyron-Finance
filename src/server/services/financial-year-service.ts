...
// Master Implementation Tracker — Epic E1, Finding #011. A journal still
// Draft/Submitted/Approved and dated within the year being closed has
// not reached the General Ledger yet — closing the year out from under
// it either strands it forever (the Posting Engine's own Financial
// Period check already refuses to post into a Closed year) or, worse,
// leaves someone assuming the year's books are final when a real
// transaction for that period is still pending. Pure and exported so
// the check is directly unit-testable without a live database.
export function findOpenJournalsInYear(journals: Journal[], year: Pick<FinancialYear, "startDate" | "endDate">): Journal[] {
  return journals.filter(
    (j) => (j.status === "Draft" || j.status === "Submitted" || j.status === "Approved") && j.journalDate >= year.startDate && j.journalDate <= year.endDate,
  );
}

/** Was a bare, unvalidated, unconfirmed status flip — see #011 in the
 * Master Implementation Tracker. Confirmation is the caller's job (the
 * API route/UI layer, matching how every other consequential action in
 * this codebase is confirmed); this function's job is the accounting
 * validation that must never be skippable: a year cannot be closed
 * while it still has unposted journals dated within it. */
export async function closeFinancialYear(companyId: string, financialYearId: number): Promise<FinancialYear> {
  const years = await repo.listFinancialYears(companyId);
  const year = years.find((y) => y.id === financialYearId);
  if (!year) throw new ValidationError(`No financial year with id ${financialYearId}.`);
  if (year.status === "Closed") throw new ValidationError(`Financial year ${year.yearLabel} is already closed.`);

  const allJournals = await journalRepo.listJournals(companyId);
  const openJournals = findOpenJournalsInYear(allJournals, year);
  if (openJournals.length > 0) {
    throw new ValidationError(
      `Cannot close ${year.yearLabel} — ${openJournals.length} journal${openJournals.length === 1 ? " is" : "s are"} dated within this year but not yet Posted (still Draft, Submitted, or Approved). Post, reject, or cancel ${openJournals.length === 1 ? "it" : "them"} first.`,
    );
  }
