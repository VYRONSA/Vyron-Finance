...
/** Finding #031 — mirrors `customer-receipt-service.ts::unallocateReceipt`
 * exactly. */
export async function unallocatePayment(companyId: string, allocationId: number, performedBy: string): Promise<void> {
  const deleted = await repo.deletePaymentAllocation(companyId, allocationId);
  if (!deleted) throw new NotFoundError(`No allocation with id ${allocationId}.`);

  await billRepo.reduceBillOutstanding(companyId, deleted.billId, -deleted.amountAllocated);
  await recordOverride(companyId, {
    itemType: "SupplierPaymentAllocation",
    itemId: allocationId,
    fieldName: "billId",
    oldValue: String(deleted.billId),
    newValue: null,
    reason: `Unallocated ${deleted.amountAllocated} from bill ${deleted.billId}`,
    performedBy,
  });
}
