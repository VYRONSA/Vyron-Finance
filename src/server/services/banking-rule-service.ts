 ...
 * identical active Banking Rule twice." Production forensic finding:
 * two byte-for-byte identical "Auto: Fish → Supplier" rules (ids 7 and
 * 9) existed side by side — `findExactDuplicateRule` (Transaction
 * Explorer Redesign, Phase 1) already existed and already implemented
 * the correct company-scoped, condition+action comparison, but it was
 * only ever wired into a non-blocking client-side pre-check (the inline
 * "Set Rule" checkbox's duplicate badge, `banking-rules/check-duplicate/
 * route.ts`) — nothing ever stopped the actual creation call from
 * proceeding regardless of that check's result. Carries the existing
 * rule so every caller (the route, the client) can build a precise,
 * identifying message without a second lookup. */
export class DuplicateRuleError extends Error {
  constructor(public readonly existingRule: BankingRule) {
    super(`An identical active rule already exists: #${existingRule.id} "${existingRule.name}".`);
  }
}
...
/** Phase 41, Part 2/4 — validate, THEN check for a duplicate, and STOP
 * before ever inserting if one is found. This is the ONE function every
 * rule-creation entry point in the app calls (the inline "Set Rule" flow
 * via `POST /banking-rules`, and the full Banking Rules admin page's
 * "New Rule" form uses the same route) — enforcing it here, not in each
 * caller, is what makes the protection apply everywhere at once. CSV
 * import (`importBankingRulesCsv` below) is deliberately NOT routed
 * through this function — it already has its own, different dedup
 * strategy (match-by-name, update-vs-create) suited to bulk import,
 * unrelated to this single-rule creation path. */
export async function createBankingRule(companyId: string, input: repo.NewBankingRule): Promise<BankingRule> {
  ...
  const duplicate = await findExactDuplicateRule(companyId, input.domain as RuleDomain, input.ruleType, input.conditions, input.actions);
  if (duplicate) throw new DuplicateRuleError(duplicate);
...
/** Phase 32 — the "Download Template" button's header source. Identical
 * to `CSV_HEADERS` above (unlike Chart of Accounts, every export column
 * here IS also read on import — see `parseBankingRulesCsv` below), kept
 * as a distinct exported name so this file's export/import contracts
 * stay independently named even though they happen to match today. */
export const BANKING_RULES_IMPORT_TEMPLATE_HEADERS = CSV_HEADERS;
...
/** Phase 32A — a blank Priority cell means "use the default" (100, the
 * same default the repository itself falls back to via `priority ?? 100`).
 * A non-blank cell that isn't a valid number must reject the row: the old
 * `cell ? Number(cell) : 100` let garbage text through as `Number(x)`,
 * silently storing `NaN` into a numeric DB column (and its own
 * `.order("priority")` sort) rather than defaulting OR rejecting. */
function parseOptionalNumber(raw: string, fallback: number): { ok: true; value: number } | { ok: false } {
...
export function parseBankingRulesCsv(csvText: string): { rows: BankingRuleImportRow[]; errors: string[] } {
  ...
  dataRows.forEach((raw, i) => {
    ...
    const priorityRaw = cell(raw, "priority");
    const priority = parseOptionalNumber(priorityRaw, 100);
    if (!priority.ok) return void errors.push(`Row ${rowNumber}: Priority "${priorityRaw}" is not a valid number.`);
