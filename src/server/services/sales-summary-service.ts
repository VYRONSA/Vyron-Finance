 ...
 * same convention `customer-financial-service.ts` uses. Exported so other
 * pure detectors needing the same customer-revenue definition (e.g.
 * `customer-concentration-detector.ts`) reuse it rather than re-deriving
 * their own Credit Note treatment. */
export function signedTotal(invoice: Pick<SalesInvoice, "documentType" | "total">): number {
