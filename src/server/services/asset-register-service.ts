...
import { buildAssetDisposalJournalLines, resolveAssetClassAccounts } from "@/server/assets/asset-lifecycle-engine";
...
export type TransferInput = {
  ...
  location?: string;
  custodian?: string;
  /** Finding #159 — Improve already recorded a real reason to the audit
   * trail (its `description` param); Transfer never had anywhere to put
   * one, so its lifecycle event description was permanently the fixed
   * string "Asset transferred." Optional, matching Edit Details' own
   * "recorded to audit trail" framing rather than making it mandatory. */
  reason?: string;
};
...
export async function transferAsset(companyId: string, assetId: number, input: TransferInput, performedBy: string): Promise<FixedAsset> {
  ...
  await lifecycleRepo.createAssetLifecycleEvent(companyId, {
    ...
    description: reason ? `Asset transferred. ${reason}` : "Asset transferred.",
...
export async function revalueAsset(companyId: string, assetId: number, delta: number, reason: string | undefined, performedBy: string): Promise<FixedAsset> {
  const asset = await requireAsset(companyId, assetId);
  ...
  // Finding #159 — mirrors `improveAsset`'s already-real reason-to-audit-
  // trail pattern; Revalue/Impair had no reason field at all before this.
  const trimmedReason = reason?.trim();
  await lifecycleRepo.createAssetLifecycleEvent(companyId, {
    assetId,
    eventType,
    eventDate: new Date().toISOString().slice(0, 10),
    description: trimmedReason ? `${eventType} of ${amount} — ${trimmedReason}` : `${eventType} of ${amount}`,
    amount,
    journalId,
    performedBy,
  });
...
export async function disposeAsset(
  ...
  // Finding #050 — resolved against the asset's own class instead of
  // hardcoded accounts.
  const assetClass = asset.assetClassId !== null ? await assetClassRepo.getAssetClass(companyId, asset.assetClassId) : null;
  const accountCodes = resolveAssetClassAccounts(assetClass);

  const built = buildAssetDisposalJournalLines(asset.cost, asset.accumulatedDepreciation, asset.accumulatedImpairment, proceeds, paymentAccountCode, accountCodes, description || `${eventType} of ${asset.assetNumber}`);
