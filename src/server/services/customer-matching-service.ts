...
export type CustomerMatchingWorkspaceData = {
  ...
  // Finding #148 — the Statement panel's customer selector showed raw
  // IDs ("Customer #4") with no name; a minimal id/name list is cheaper
  // than the UI re-deriving names from invoices/receipts (neither of
  // which carries a display name). Phase 20C — vatNumber/registrationNumber
  // added (both already real Customer fields) so the Statement document
  // can show them without a second customer fetch.
  customers: { id: number; name: string; vatNumber: string; registrationNumber: string }[];
};

export async function getCustomerMatchingWorkspace(companyId: string): Promise<CustomerMatchingWorkspaceData> {
  const [allInvoices, receipts, customers] = await Promise.all([listSalesInvoices(companyId), listCustomerReceipts(companyId), listCustomers(companyId)]);
  return {
    ...
    customers: customers.map((c) => ({ id: c.id, name: c.name, vatNumber: c.vatNumber, registrationNumber: c.registrationNumber })),
  };
}
