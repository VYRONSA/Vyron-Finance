...
export async function getDuplicateFindings(companyId: string, referenceDateIso: string): Promise<DuplicateFinding[]> {
  ...
  // Phase 33A — `listSuppliers` returns every supplier regardless of
  // status. Without this filter, a supplier just merged away by
  // `mergeService.mergeSuppliers` (soft-deactivated, `status: "Inactive"`)
  // would still be fed into `findDuplicateParties` below and keep
  // matching its survivor's name forever — Duplicate Detection would
  // never stop reporting a pair that has already been resolved. An
  // Inactive supplier is, by definition, no longer a live master record
  // competing for "which one is real."
  const activeSuppliers = suppliers.filter((s) => s.status === "Active");

  const findings = buildDuplicateFindings(companyId, {
    customers, suppliers: activeSuppliers, merchants, stockItems,
