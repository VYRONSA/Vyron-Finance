...
/** Finding #057 — a Draft or Submitted requisition previously had no way
 * to be withdrawn, mirroring every sibling document's own Cancel action. */
export const cancelRequisition = (companyId: string, requisitionId: number) => transitionRequisition(companyId, requisitionId, "Cancelled");

/** Finding #166 — mirrors `quotation-service.ts::cloneQuotationAsDraft`
 * exactly: a Rejected requisition was a dead end. */
export async function cloneRequisitionAsDraft(companyId: string, requisitionId: number): Promise<PurchaseRequisition> {
  const requisition = await repo.getPurchaseRequisition(companyId, requisitionId);
  if (!requisition) throw new NotFoundError(`No purchase requisition with id ${requisitionId}.`);
  if (requisition.status !== "Rejected") {
    throw new ValidationError(`Only a Rejected requisition can be cloned (current status: ${requisition.status}).`);
  }
  return repo.createPurchaseRequisition(companyId, {
    requestedBy: requisition.requestedBy,
    requisitionDate: new Date().toISOString().slice(0, 10),
    notes: requisition.notes,
    lines: requisition.lines.map((l) => ({ description: l.description, quantity: l.quantity, estimatedUnitPrice: l.estimatedUnitPrice })),
  });
}
