...
export function supplierFromRow(row: SupplierRow): Supplier {
  return {
    ...
    spendingLimit: row.spending_limit ?? 0,
  };
}
...
export type BankTransactionRow = {
  ...
  is_split: boolean;
  // Bank Accounting Posting / duplicate-preserving import — migration 0092.
  posted_flag: boolean;
  posted_at: string | null;
  posting_batch_id: number | null;
  source_occurrence: number;
  // Human-review hold — migration 0094.
  review_hold: boolean;
...
export function bankTransactionFromRow(row: BankTransactionRow): BankTransactionRecord {
  return {
    ...
    postedFlag: row.posted_flag ?? false,
    postedAt: row.posted_at ?? null,
    postingBatchId: row.posting_batch_id ?? null,
    sourceOccurrence: row.source_occurrence ?? 1,
    reviewHold: row.review_hold ?? false,
 ...
 * the FULL update, used only when the transaction is not yet
 * accounting-protected by another engine (see
 * `isSupplierReconAccountingProtected` in
 * `supplier-reconciliation-repository.ts`). Phase 29B — now also sets
 * `allocation_type: 'S'` whenever a supplier was actually identified: at
 * this point in the two-phase Match-then-Allocate pipeline no GL account
 * exists yet (Allocation, which may supersede this with 'G', always runs
 * after Matching — see `runAllocationForCompany`'s own doc comment), so
 * the supplier is genuinely the row's only known accounting target so
 * far. Leaves `allocation_type` untouched when nothing was identified at
 * all (`matchedSupplierId === null` — Unmatched), consistent with
 * `isAllocationMissing`'s own "nothing to represent yet" treatment
...
export function matchResultToUpdate(result: MatchResult) {
  return {
    ...
    ...(result.matchedSupplierId !== null ? { allocation_type: "S" as const } : {}),
  };
}

/** Phase 29B — the NARROW update used when a transaction is already
 * accounting-protected (manually allocated, AI-accepted, Rule-allocated)
 * but has not yet had a supplier identified at all
 * (`matched_supplier_id IS NULL`). Deliberately writes ONLY the
 * "who is this" identification fields — never `allocation_status`,
 * `required_action`, or `allocation_type` — so enriching a transaction
 * with its matched supplier/bill can never overwrite or misrepresent an
 * existing, unrelated accounting decision (e.g. a transaction already
 * manually allocated to GL 5000 gains its identified supplier without
 * its status becoming "Matched" or its `allocation_type` becoming 'S'). */
export function matchResultEnrichmentUpdate(result: MatchResult) {
  return {
    matched_supplier_id: result.matchedSupplierId,
    matched_bill_id: result.matchedBillId,
    confidence_score: result.confidence,
...
/** Fields updated on `ae_bank_transactions` after an Allocation Engine
 * pass. Phase 29B — now also sets `allocation_type`, following the SAME
 * G/S/C vocabulary already established by `applyRuleActions`
 * (`transaction-explorer-repository.ts`) and `initialEdit`
 * (`transaction-grid.tsx`) — but with GL taking precedence over Supplier
 * here specifically, the opposite of those two: by the time this engine
 * derives a real `suggested_gl_account` (from the bill's own GL or the
 * supplier's defaults), that IS the genuinely-decided accounting target,
 * more specific than the earlier, coarser supplier identification —
 * `allocation_type` must represent THAT, not silently regress to 'S'.
 * Falls back to 'S' only when no GL was derivable at all but a supplier
 * is still known (e.g. "supplier matched but has no default GL/VAT
 * configured" — `allocation-engine.ts`'s own documented Rule 2 fallback),
 * and leaves it untouched when genuinely nothing is known
 * (`STATUS_UNALLOCATED`, no supplier at all). */
export function allocationResultToUpdate(result: AllocationResult) {
  return {
    ...
    required_action: result.requiredAction,
    ...(result.glAccount !== null ? { allocation_type: "G" as const } : result.supplierId !== null ? { allocation_type: "S" as const } : {}),
  };
}

/** Phase 29B — forensic review finding: `applyMatchResults`/
 * `applyAllocationResults` had NO protection at all against overwriting a
 * transaction another engine had already genuinely decided (manually
 * allocated, an accepted/automatic AI allocation, or a Banking Rule
 * match) — only `journal_id IS NULL` and company scoping. Pure and
 * exported so this is directly unit-testable and so the repository's
 * actual database WHERE clause (which must express the identical
 * condition at the SQL level for real, race-safe protection — see
 * `supplier-reconciliation-repository.ts`) can be verified against one
 * single source of truth for what "already accounting-decided" means.
 *
 * Deliberately does NOT include `matched_supplier_id`/`matched_bill_id`
 * in this check — those are "who is this" identification, not an
 * accounting decision (GL/VAT/status), and Supplier Reconciliation is
 * explicitly still allowed to enrich them via `matchResultEnrichmentUpdate`
 * even on an accounting-protected row, as long as no supplier is
 * identified yet (`matched_supplier_id IS NULL` — checked separately,
 * at the repository's own enrichment-branch WHERE clause). */
export function isSupplierReconAccountingProtected(t: Pick<BankTransactionRecord, "isManualOverride" | "ruleId" | "allocationMethod">): boolean {
  return t.isManualOverride || t.ruleId !== null || t.allocationMethod === "Future AI" || t.allocationMethod === "Manual";
}

/** Phase 29B — "same result twice must not create duplicate history."
 * Compares every field the FULL `matchResultToUpdate` would write
 * against the transaction's current state; `true` means a real,
 * history-worthy change. Deliberately mirrors exactly the fields that
 * mapper writes — kept as a separate function (not derived from it)
 * because comparing "would this value differ" is a distinct concern
 * from "what should the update object contain." */
export function hasMatchResultChanged(
  previous: Pick<BankTransactionRecord, "allocationStatus" | "matchedSupplierId" | "matchedBillId" | "confidenceScore" | "requiredAction">,
  result: MatchResult,
): boolean {
  const newStatus = result.status === "Unmatched" ? "Unallocated" : result.status;
  return (
    previous.allocationStatus !== newStatus ||
    previous.matchedSupplierId !== result.matchedSupplierId ||
    previous.matchedBillId !== result.matchedBillId ||
    previous.confidenceScore !== result.confidence ||
    previous.requiredAction !== result.requiredAction
  );
}

/** Phase 29B — the identical no-op guard for the Allocation Engine's
 * FULL update, comparing every field `allocationResultToUpdate` writes. */
export function hasAllocationResultChanged(
  previous: Pick<BankTransactionRecord, "allocationStatus" | "suggestedGlAccount" | "suggestedVatCode" | "allocationMethod" | "requiredAction">,
  result: AllocationResult,
): boolean {
  return (
    previous.allocationStatus !== result.status ||
    previous.suggestedGlAccount !== result.glAccount ||
    previous.suggestedVatCode !== result.vatCode ||
    previous.allocationMethod !== result.allocationMethod ||
    previous.requiredAction !== result.requiredAction
  );
}
