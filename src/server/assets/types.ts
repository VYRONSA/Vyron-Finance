...
export type AssetClass = {
  ...
  isActive: boolean;
  createdAt: string;
  /** Finding #050 — per-class GL account overrides. Null means "use the
   * platform default" (see `asset-lifecycle-engine.ts`'s own
   * `DEFAULT_*_ACCOUNT_CODE` constants) — every asset posted identically
   * before this existed, so a class that never sets these must keep
