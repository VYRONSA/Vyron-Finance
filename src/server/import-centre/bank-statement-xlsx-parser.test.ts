...
describe("parseBankStatementXlsx", () => {
  ...
  // Phase 32A — mirrors the CSV parser's fix: a non-blank Balance/VAT that
  // fails to parse must reject the row, never silently vanish as null.
  it("flags a non-numeric Balance as an Invalid Amount exception rather than silently dropping it", async () => {
    const buffer = await buildWorkbookBuffer([["2026-06-15", "INV-1", "Line rental", "Telkom SA", 450, 0, "not-a-number", "Main Account", "", "", ""]]);
    const result = await parseBankStatementXlsx(buffer, "statement.xlsx", "BATCH-1");
    expect(result.transactions).toHaveLength(0);
    expect(result.exceptions).toMatchObject([{ exceptionType: "Invalid Amount", beneficiary: "Telkom SA" }]);
    expect(result.exceptions[0].description).toContain("Invalid Balance value");
  });

  it("flags a non-numeric VAT as an Invalid Amount exception rather than silently dropping it", async () => {
    const buffer = await buildWorkbookBuffer([["2026-06-15", "INV-1", "Line rental", "Telkom SA", 450, 0, 10000, "Main Account", "garbage", "", ""]]);
    const result = await parseBankStatementXlsx(buffer, "statement.xlsx", "BATCH-1");
    expect(result.transactions).toHaveLength(0);
    expect(result.exceptions).toMatchObject([{ exceptionType: "Invalid Amount", beneficiary: "Telkom SA" }]);
    expect(result.exceptions[0].description).toContain("Invalid VAT value");
  });

  it("still treats a genuinely blank Balance/VAT as null, not an error", async () => {
    const buffer = await buildWorkbookBuffer([["2026-06-15", "INV-1", "Line rental", "Telkom SA", 450, 0, "", "Main Account", "", "", ""]]);
    const result = await parseBankStatementXlsx(buffer, "statement.xlsx", "BATCH-1");
    expect(result.exceptions).toEqual([]);
    expect(result.transactions[0]).toMatchObject({ balance: null, vat: null });
  });
});
