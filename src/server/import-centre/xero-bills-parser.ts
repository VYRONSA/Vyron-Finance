...
/** Phase 32 — one readable, human-friendly header per field for the
 * "Download Template" button, each verified against `FIELD_ALIASES`
 * above so every header here is a real, already-recognised alias (not a
 * new one invented for the template) — "Contact Name"/"Invoice Number"
 * are simply the more readable member of their own alias list, matching
 * common Xero export conventions this parser was already ported from. */
export const BILLS_IMPORT_TEMPLATE_HEADERS = [
  "Contact Name", "Invoice Number", "Reference", "Type", "Invoice Date",
  "Due Date", "Status", "Currency", "Subtotal", "VAT", "Total", "Amount Due",
];
...
// #143. When the source file has no currency column, this used to
// silently default to GBP regardless of the company's actual currency —
// a hardcoded literal, not a mishandled read. Defaults to "ZAR" only to
// preserve this function's existing call signature for any caller that
// doesn't (yet) pass one; `import-service.ts` passes the company's own
// `baseCurrencyCode`.
export function parseBillsCsv(fileText: string, filename: string, importBatch: string, defaultCurrency = "ZAR"): BillsParseResult {
  ...
  if (missingRequired.length > 0) {
    exceptions.push({
      ...
      description: `Missing required column(s): ${missingRequired.sort().join(", ")} (headers found: ${headers.join(", ")}). Download the Bills Import Template and use its column headers.`,
    });
    return { transactions, exceptions, outcome: { filename, documentTypeDetected: null, rowsRead: 0, rowsImported: 0, rowsSkipped: 0 } };
  }
