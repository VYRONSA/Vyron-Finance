...
// Phase 32 — exported so the "Download Template" button
// (`import-upload-card.tsx`) reads the SAME array the parser actually
// checks against, rather than a second, hand-copied list that could
// drift from it.
...
export function parseBankStatementCsv(fileText: string, filename: string, importBatch: string): BankStatementParseResult {
  ...
  for (let offset = 0; offset < dataRows.length; offset++) {
    ...
    try {
      ...
      // Phase 32A — Balance and VAT are optional columns, but a non-blank
      // value that fails to parse is a real data problem, not "nothing
      // was provided" — `parseAmount` alone can't tell those two cases
      // apart (both return `null`), so the presence check has to happen
      // here, same principle as the Debit/Credit guard above. Without
      // this, a mistyped VAT figure would silently vanish as if the
      // column were empty, with no error ever shown to the accountant.
      const balanceRaw = cleanText(cell(6));
