...
export type Company = {
  ...
  createdAt: string;
  // Phase 20D — Company Contact, Tax & Document Information. Additional
  // profile fields alongside the authoritative name/registrationNumber/
  // address above, never a duplicate of them. Empty string means "not
  // provided" (same convention as registrationNumber/address already
  // use), never null.
  tradingName: string;
  vatNumber: string;
  telephone: string;
  ...
  // Xero Client Import — migration 0091. Structured address detail
  // alongside the free-text `address`/`postalAddress` lines above
  // (matching the line1/city/region/postalCode/country shape
  // `customer_addresses`/`supplier_addresses` already use), so an
  // imported organisation's city/province/postal code/country survive as
  // queryable fields, not just concatenated text.
  city: string;
  province: string;
  postalCode: string;
  country: string;
