...
/** Phase 30 — `account_code` is a TEXT column (`ACCOUNT_CODE_RE` in
 * `chart-of-accounts-service.ts` allows letters/dots/dashes/underscores
 * too, not just digits), so a plain string `.order("account_code")` /
 * `.localeCompare()` sort is LEXICOGRAPHIC, not numeric — it only ever
 * looked correct because every real account code today happens to be
 * the same length (4 digits). Lexicographic order silently breaks the
 * moment a differently-sized code exists (e.g. "500" would sort before
 * "1000" is intended to, and "10000" would sort before "2000"
 * lexicographically despite being numerically larger).
 *
 * This is the ONE shared, pure, exported comparator every GL-account
 * ordering location in the app should use instead — numeric comparison
 * when both codes parse as real numbers (the normal case), falling back
 * to a stable lexicographic comparison only for the genuinely
 * non-numeric codes the format still permits, so this never throws or
 * produces a NaN-driven comparison. */
...
/** Convenience wrapper for the extremely common "I have a list of
 * accounts (or anything carrying an `accountCode`), give me a NEW array
 * in ascending GL-code order" case — never mutates its input. */
export function sortByGlAccountCode<T extends { accountCode: string }>(items: T[]): T[] {
  return [...items].sort((a, b) => compareGlAccountCodes(a.accountCode, b.accountCode));
}
