...
describe("detectVatRateConflict", () => {
  function history(entries: Partial<VatRateHistoryEntry>[]): VatRateHistoryEntry[] {
    return entries.map((e, i) => ({ id: i + 1, vatTreatmentId: 1, rate: 15, effectiveFrom: "2020-01-01", effectiveTo: null, createdAt: "2020-01-01T00:00:00Z", createdBy: "System", ...e }));
  }

  // A rate change: 15% up to end of 2024, 16% from 2025-01-01 onward.
  const RATE_CHANGE_HISTORY = history([
    { rate: 15, effectiveFrom: "2020-01-01", effectiveTo: "2024-12-31" },
    { rate: 16, effectiveFrom: "2025-01-01", effectiveTo: null },
  ]);

  it("flags a document whose VAT doesn't match the rate that was actually in effect on its own date", () => {
    // Dated during the 15% period but charged at 16% — a real conflict.
    const signals = detectVatRateConflict(
      [doc({ id: 1, date: "2024-06-01", grossAmount: 1160, vatAmount: 160 })],
      new Map([["Standard Rated", RATE_CHANGE_HISTORY]]),
    );
    expect(signals.get(1)?.kind).toBe("high-risk");
  });

  it("does NOT flag a legitimate historical document correctly priced at the OLD rate — the exact gap this detector closes vs. a flat 'today' rate map", () => {
    // Dated during the 15% period, correctly charged at 15%.
    const doc1 = doc({ id: 1, date: "2024-06-01", grossAmount: 1150, vatAmount: 150 });
    const rateConflict = detectVatRateConflict([doc1], new Map([["Standard Rated", RATE_CHANGE_HISTORY]]));
    expect(rateConflict.size).toBe(0);

    // Whereas the flat, "today's rate only" detector WOULD incorrectly
    // flag this same legitimate document if the caller naively compared
    // it against today's 16% rate — proving these are genuinely distinct.
    const suspicious = detectSuspiciousVatValues([doc1], new Map([["Standard Rated", 16]]));
    expect(suspicious.size).toBe(1);
  });

  it("correctly flags a document dated in the new-rate period but still charged at the old rate", () => {
    const signals = detectVatRateConflict(
      [doc({ id: 1, date: "2025-06-01", grossAmount: 1150, vatAmount: 150 })],
      new Map([["Standard Rated", RATE_CHANGE_HISTORY]]),
    );
    expect(signals.get(1)?.kind).toBe("high-risk");
  });

  it("skips a document whose treatment has no rate history at all", () => {
    const signals = detectVatRateConflict([doc({ id: 1, vatTreatmentCode: "Unknown" })], new Map());
    expect(signals.size).toBe(0);
  });

  it("skips a zero-value document", () => {
    const signals = detectVatRateConflict([doc({ id: 1, grossAmount: 0, date: "2024-06-01" })], new Map([["Standard Rated", RATE_CHANGE_HISTORY]]));
    expect(signals.size).toBe(0);
  });

  it("does not flag a correctly reconciled document", () => {
    const signals = detectVatRateConflict(
      [doc({ id: 1, date: "2025-06-01", grossAmount: 1160, vatAmount: 160 })],
      new Map([["Standard Rated", RATE_CHANGE_HISTORY]]),
    );
    expect(signals.size).toBe(0);
  });
