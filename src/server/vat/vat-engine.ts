...
/** Pure — Finding #203. A plain two-line DR-liability/CR-bank journal
 * settling part (or all) of a VAT Return's VAT Control (2300) balance —
 * the same gross two-line shape `posting_rules` already model for
 * 'Supplier Payment' (see `seed_vat_payment_posting_rule` in
 * `0072_vat_payments_and_brought_forward.sql`), expressed here as a pure
 * function purely so `vat-payment-service.ts` never has to round-trip
 * through the Posting Rules repository just to settle a fixed two-account
 * shape. */
/** Pure — Findings #202/#203's shared correctness core: how much of a
 * VAT Return's `netPayable` is still actually outstanding once existing
 * `VatPayment`s are accounted for. The one place this subtraction
 * happens, so a settled-but-unpaid liability is never double-counted —
 * used both to derive the next period's `broughtForward` (#202) and to
 * cap how much a new payment may settle (#203). Never negative — an
 * overpayment is a distinct scenario this function does not model. */
export function computeOutstandingBalance(netPayable: number, alreadyPaid: number): number {
  const outstanding = Math.round((netPayable - alreadyPaid) * 100) / 100;
  return outstanding > 0 ? outstanding : 0;
}

export function buildVatPaymentJournalLines(amount: number, description: string): SettlementJournalResult {
  ...
  return {
    ok: true,
    lines: [
      { accountCode: "2300", debit: value, credit: 0, description },
      { accountCode: "1000", debit: 0, credit: value, description },
    ],
  };
}
