...
import {
  buildVatPaymentJournalLines,
  buildVatSettlementJournalLines,
  computeOutstandingBalance,
  isVatSplitConsistentWithType,
...
describe("buildVatPaymentJournalLines", () => {
  it("debits VAT Control and credits Bank for the paid amount", () => {
    const result = buildVatPaymentJournalLines(400, "VAT payment");
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(result.lines).toEqual([
      { accountCode: "2300", debit: 400, credit: 0, description: "VAT payment" },
      { accountCode: "1000", debit: 0, credit: 400, description: "VAT payment" },
    ]);
  });

  it("returns ok:false for a zero or negative amount", () => {
    expect(buildVatPaymentJournalLines(0, "VAT payment").ok).toBe(false);
    expect(buildVatPaymentJournalLines(-50, "VAT payment").ok).toBe(false);
  });
});

describe("computeOutstandingBalance", () => {
  it("subtracts what's already been paid from the net payable", () => {
    expect(computeOutstandingBalance(1000, 400)).toBe(600);
  });

  it("never double-counts a fully settled liability — floors at zero, never negative", () => {
    expect(computeOutstandingBalance(1000, 1000)).toBe(0);
    expect(computeOutstandingBalance(1000, 1500)).toBe(0);
  });

  it("treats an unpaid prior liability as fully outstanding when nothing has been paid", () => {
    expect(computeOutstandingBalance(1000, 0)).toBe(1000);
  });
});
