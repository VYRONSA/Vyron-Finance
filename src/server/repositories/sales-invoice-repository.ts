...
/** Finding #165 — `glAccount`/`vatCode`/`discount`/`netAmount`/`vatAmount`
 * are optional per-line capture, informational alongside the invoice's
 * own header-level `vatTreatmentCode`/`subtotal`/`vatAmount`/`total` —
 * deliberately NOT rolled up into those header figures (which drive the
 * real GL posting via `buildJournalFromEvent`), to avoid changing
 * Invoice accounting math as part of adding line-level visibility. */
export type NewSalesInvoiceLine = {
  description: string;
  quantity: number;
  unitPrice: number;
