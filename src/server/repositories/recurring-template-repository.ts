...
/** Phase 25K crash-recovery guard — `generateFromTemplate` writes this
 * Success row BEFORE it advances `next_run_date`/`last_run_date`. A
 * crash in that gap leaves the template still "due," so a retry (the
 * scheduler's stale-Running reclaim sweep, or a manual re-run) would
 * call `dispatchGeneration` a second time and create a duplicate
 * document + duplicate posted journal. Any Success row generated after
 * the template's last confirmed `last_run_date` is exactly that
 * dangling record from an interrupted run — its presence means
 * generation already happened and only the advance step is missing. */
export async function findGeneratedDocumentSince(companyId: string, templateId: number, sinceIso: string | null): Promise<GeneratedDocument | null> {
  const supabase = await createClient();
  let query = supabase
    .from("generated_documents")
    .select("*")
    ...
    .eq("status", "Success");
  if (sinceIso) query = query.gt("generated_at", sinceIso);
  const { data, error } = await query.order("generated_at", { ascending: false }).limit(1).returns<GeneratedDocumentRow[]>();
  if (error) throw error;
  return data.length > 0 ? generatedDocumentFromRow(data[0]) : null;
}
