...
/** Finding #133/#134/#138 (RC-6) — same batching fix as
 * `billing-event-repository.ts::listBillingEventsForCompanies`: Platform
 * Overview's "Notifications" panel only needs the global top-N most
 * recent unread notifications across every company the user can see. */
export async function listNotificationsForCompanies(companyIds: string[], unreadOnly = false, limit = 5): Promise<AppNotification[]> {
  if (companyIds.length === 0) return [];
  const supabase = await createClient();
  let query = supabase.from("notifications").select("*").in("company_id", companyIds);
  if (unreadOnly) query = query.eq("is_read", false);
  const { data, error } = await query.order("created_at", { ascending: false }).limit(limit).returns<NotificationRow[]>();
  if (error) throw error;
  return data.map(notificationFromRow);
