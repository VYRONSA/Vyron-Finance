...
export async function createBankAccount(companyId: string, input: CreateBankAccountInput): Promise<BankAccount> {
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      opening_balance_reference: input.openingBalanceReference,
 ...
 * `created: true` on the result lets callers surface it — this used to
 * happen with no signal at all, so a mistyped account number on a bank
 * statement silently produced a new, empty "phantom" bank account with
 * no warning anywhere. */
export async function getOrCreateBankAccountByNumber(companyId: string, accountNumber: string): Promise<{ account: BankAccount; created: boolean }> {
  const existing = await findBankAccountByNumber(companyId, accountNumber);
  if (existing) return { account: existing, created: false };
  const account = await createBankAccount(companyId, {
    accountNumber,
