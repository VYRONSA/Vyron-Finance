...
// Finding #109 — the one list query in this repository had no cap at
// all (unlike ~38 other repositories in this codebase), so a company
// with a large asset base would fetch its entire register on every page
// load. Same LIST_CAP convention used everywhere else.
...
export async function listFixedAssets(companyId: string, status?: AssetStatus): Promise<FixedAsset[]> {
  ...
  const { data, error } = await query.order("asset_number").limit(LIST_CAP).returns<FixedAssetRow[]>();
