...
/** Finding #031 — mirrors `customer-receipt-repository.ts::deleteReceiptAllocation`
 * exactly, including relying on `supplier_payment_allocations`' own RLS
 * (scoped via the parent payment's `company_id`) rather than a second
 * lookup. */
export async function deletePaymentAllocation(companyId: string, allocationId: number): Promise<{ billId: number; amountAllocated: number } | null> {
  const supabase = await createClient();
  const { data, error } = await supabase
    ...
    .maybeSingle<{ bill_id: number; amount_allocated: number }>();
  if (error) throw error;
  return data ? { billId: data.bill_id, amountAllocated: Number(data.amount_allocated) } : null;
}
