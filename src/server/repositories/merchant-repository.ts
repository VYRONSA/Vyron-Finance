 ...
 *
 * Phase 29C — forensic audit finding: this had NO `journal_id IS NULL`
 * guard, unlike every other write to `ae_bank_transactions` in this
 * codebase — a merge could silently re-point an already-posted
 * transaction's merchant association. Added here for the same reason as
 * everywhere else: a posted transaction's own recorded state must never
 * change after the fact. A posted transaction excluded by this guard is
 * simply left pointing at `fromMerchantId` — since the merge service
 * always calls `deleteMerchant` (the losing merchant) right after this,
 * and `matched_merchant_id references merchants (id) on delete set
 * null` (migration 0013), that reference safely becomes NULL rather
 * than dangling once the old merchant record is deleted; it is never
 * silently repointed to the new merchant instead. */
