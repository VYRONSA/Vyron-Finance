...
/** Finding #031 — the reverse of `createReceiptAllocation`. Row Level
 * Security on `customer_receipt_allocations` already scopes by the
 * parent receipt's `company_id` (see migration 0010), so a delete
 * filtered only by `id` is safely company-scoped without a separate
 * lookup — a cross-company/nonexistent id simply matches zero rows.
 * `.select()` on the delete returns what was removed in the same round
 * trip, since the caller needs `invoiceId`/`amountAllocated` to restore
 * the invoice's outstanding balance. */
export async function deleteReceiptAllocation(companyId: string, allocationId: number): Promise<{ invoiceId: number; amountAllocated: number } | null> {
  const supabase = await createClient();
  const { data, error } = await supabase
    ...
    .maybeSingle<{ invoice_id: number; amount_allocated: number }>();
  if (error) throw error;
  return data ? { invoiceId: data.invoice_id, amountAllocated: Number(data.amount_allocated) } : null;
}
