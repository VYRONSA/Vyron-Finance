...
/** Finding #171 (RC-16/E13) — same as `copilot-narrative-repository.ts::deleteCopilotNarrative`. */
export async function deleteCopilotBriefing(companyId: string, briefingId: number): Promise<void> {
  const supabase = await createClient();
  const { error } = await supabase.from("copilot_briefings").delete().eq("company_id", companyId).eq("id", briefingId);
  if (error) throw error;
}
