...
export async function createWorkflowInstance(companyId: string, definitionId: number, subjectType: string, subjectId: number, requestedBy: string): Promise<WorkflowInstance> {
  const supabase = await createClient();
  const { data, error } = await supabase
    .from("workflow_instances")
    .insert({ company_id: companyId, workflow_definition_id: definitionId, subject_type: subjectType, subject_id: subjectId, requested_by: requestedBy })
    .select("*")
