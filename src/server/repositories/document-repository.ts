...
/** Phase 24B — the download-side counterpart to `uploadDocumentFile`,
 * added specifically so `communication-service.ts::deliverAndFinalize`
 * can actually forward a document's real bytes to `EmailSender.send()`
 * as an attachment. No prior caller needed this (`getSignedDownloadUrl`
 * below exists only for a browser to fetch a file directly) — a
 * server-to-provider email attachment needs the real bytes in hand, not
 * a URL a third-party provider would then have to fetch itself. */
