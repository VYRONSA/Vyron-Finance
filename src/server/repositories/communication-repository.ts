...
// Finding #063 (RC-6) — capped, but with no user-visible indicator when
// the cap is actually hit; exported so the service/UI layers can derive
// a real "showing the most recent N" disclosure instead of guessing.
export const COMMUNICATIONS_LIST_CAP = 500;
 ...
 * atomically flips a row to `Sending`, but nothing previously ever
 * revisited it if the process died before `deliverAndFinalize` reached a
 ...
 * forever. `updated_at` is stamped by the claim itself (and by nothing
 * else that ever sets `Sending`), so it reliably reflects "when this row
 * was claimed" — no new column needed. */
export async function listStaleSendingCommunications(companyId: string, staleBeforeIso: string): Promise<CommunicationRecord[]> {
  const supabase = await createClient();
  const { data, error } = await supabase
    .from("communications")
    .select("*")
...
/** Atomically reclaims one stale-`Sending` communication back to
 * `Failed` with real retry/backoff accounting, guarded by
 * `WHERE status = 'Sending'` — the same conditional-claim discipline as
 * `claimCommunicationForSending`, just in the other direction. If the
 * real send actually finishes (or another reclaim pass gets there
 * first) a split second before this runs, the UPDATE touches zero rows
 * and this returns `false` — the row's real, legitimate outcome is
 * never overwritten. */
export async function reclaimStaleSendingCommunication(
  companyId: string,
  id: number,
  fields: { retryCount: number; failureReason: string; nextRetryAt: string | null },
): Promise<boolean> {
  const supabase = await createClient();
  const { data, error } = await supabase
    .from("communications")
    .update({
      ...
      failure_reason: fields.failureReason,
      next_retry_at: fields.nextRetryAt,
 ...
 * `transaction-explorer-repository.ts::applyAiClassification` — the
 * WHERE clause repeats `listPendingCommunications`' own eligibility
 * condition (`status in ('Queued','Failed')`), so two overlapping
 * `processCommunicationQueue` runs (e.g. an overlapping scheduler
 * invocation) can both read the same row as "due," but only ONE of
 * them will actually flip it to `Sending` — the loser's `UPDATE`
 * touches zero rows and `.maybeSingle()` returns null, which the
 * caller treats as "already claimed elsewhere, skip" rather than
 * sending a second time. */
export async function claimCommunicationForSending(companyId: string, id: number): Promise<CommunicationRecord | null> {
  const supabase = await createClient();
  const { data, error } = await supabase
    .from("communications")
