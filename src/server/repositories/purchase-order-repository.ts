 ...
 * `sales-order-repository.ts::incrementOrderLineQuantity`, including its
 * Phase 25K company-ownership guard (see that function's own comment) —
 * `purchase_order_lines` has no `company_id` of its own either, so
 * ownership is confirmed via an inner join to `purchase_orders`. */
export async function incrementOrderLineQuantity(companyId: string, orderLineId: number, field: "received_quantity" | "billed_quantity", amount: number): Promise<void> {
  const supabase = await createClient();
  const { data: current, error: readError } = await supabase
    .from("purchase_order_lines")
    .select(`${field}, purchase_orders!inner(company_id)`)
    .eq("id", orderLineId)
    .eq("purchase_orders.company_id", companyId)
    .maybeSingle();
  if (readError) throw readError;
  if (!current) throw new Error(`No purchase order line with id ${orderLineId} in company ${companyId}.`);
