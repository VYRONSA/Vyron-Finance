...
export type NewBankReconciliation = {
  bankAccountId: number;
  /** The statement period's END date. */
  statementDate: string;
  /** Its START. Optional so the existing single-date callers keep
   * working; when given, the reconciliation scopes itself to the period
   * rather than to everything up to the end date. */
...
export async function createReconciliation(companyId: string, input: NewBankReconciliation): Promise<BankReconciliation> {
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      statement_period_start: input.statementPeriodStart ?? null,
      statement_opening_balance: input.statementOpeningBalance ?? 0,
