...
type CashbookGridRow = {
  ...
  reference: string;
  amount: number;
  // Finding #022 — the grid had no VAT column at all, so every
  // batch-captured row silently posted `vat: 0` even though
  // `captureCashbookReceipt`/`captureCashbookPayment` (and the journal
  // VAT split they feed) already support it. `amount` is the gross
  // bank movement; `vatAmount` is the VAT portion within it, same
  // semantics as `vatAmount` on `CashbookReceiptInput`/`CashbookPaymentInput`.
  vatAmount: number;
};
...
function emptyRow(): CashbookGridRow {
  return { key: nextRowKey(), type: "Receipt", bankAccountId: 0, transactionDate: todayIso(), glAccount: "", description: "", reference: "", amount: 0, vatAmount: 0 };
}
...
export function BatchCaptureTab({
  ...
  // Finding #081. A batch is typically single-currency in practice — the
  // first row's own bank account currency is a reasonable display
  // currency for this running total; a genuinely mixed-currency batch
  // still totals correctly, only the displayed symbol is approximate.
  const displayCurrency = bankAccounts.find((a) => a.id === rows.find((r) => r.bankAccountId)?.bankAccountId)?.currency ?? "ZAR";

  // Master Implementation Tracker — Epic E11, Finding #198 (RC-9).
  const isRowTouched = (row: CashbookGridRow) =>
    row.bankAccountId !== 0 || row.description.trim() !== "" || row.reference.trim() !== "" || row.amount !== 0 || row.glAccount.trim() !== "";
  useUnsavedChangesWarning(rows.some(isRowTouched) || transferAmount.trim() !== "" || transferDescription.trim() !== "");

  // Master Implementation Tracker — Programme 2, Root Cause RC-7,
  // Finding #219. The GL Account column previously had no connection to
  // the Chart of Accounts at all — a typo'd code saved without warning.
  const validGlCodes = useMemo(() => new Set(chartOfAccounts.filter((a) => a.isActive).map((a) => a.accountCode)), [chartOfAccounts]);
  ...
  function validateRow(row: CashbookGridRow): string | null {
    ...
    if (!validGlCodes.has(row.glAccount.trim())) return `GL account '${row.glAccount.trim()}' not found in Chart of Accounts`;
    if (!row.description.trim()) return "Description required";
    if (!row.amount || row.amount <= 0) return "Amount must be greater than zero";
    if (row.vatAmount < 0) return "VAT cannot be negative";
    if (row.vatAmount >= row.amount) return "VAT must be less than the gross amount";
    return null;
  }
  ...
  const columns = useMemo<GridColumn<CashbookGridRow>[]>(
    () => [
      ...
      { key: "vatAmount", label: "VAT", type: "number", align: "right", width: "w-28" },
    ],
    [bankAccounts],
  );
  ...
  async function handleSaveDraft() {
    ...
    const validRows = rows.filter(
      (r) => r.bankAccountId && r.transactionDate && r.glAccount.trim() && validGlCodes.has(r.glAccount.trim()) && r.description.trim() && r.amount > 0 && r.vatAmount >= 0 && r.vatAmount < r.amount,
    );
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <Card>
          <CardContent className="p-5">
            ...
            <p className="mt-1 font-mono text-2xl font-semibold tabular-nums text-vf-ink">{formatMoney(receiptsTotal, displayCurrency)}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-5">
            <p className="text-xs font-medium text-vf-ink-faint uppercase">Payments (this session)</p>
            <p className="mt-1 font-mono text-2xl font-semibold tabular-nums text-vf-ink">{formatMoney(paymentsTotal, displayCurrency)}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-5">
