...
/** Master Implementation Tracker — Epic E11, Finding #209 (RC-9). See
 * the matching persistence in `ImportUploadCard` — this panel owns the
 * transactions half of the in-progress review draft. */
export function transactionsStorageKey(companyId: string, batchId: string): string {
  return `vyron:pdf-import-transactions:${companyId}:${batchId}`;
}

function readStoredTransactions(companyId: string, batchId: string, fallback: ParsedBankTransaction[]): ParsedBankTransaction[] {
  if (typeof window === "undefined") return fallback;
  try {
    const raw = window.sessionStorage.getItem(transactionsStorageKey(companyId, batchId));
    return raw ? (JSON.parse(raw) as ParsedBankTransaction[]) : fallback;
  } catch {
    return fallback;
  }
}
...
export function PdfImportReviewPanel({
  companyId,
  preview,
  onDiscard,
  onConfirmed,
  ...
  const [transactions, setTransactions] = useState<ParsedBankTransaction[]>(() => readStoredTransactions(companyId, preview.batchId, preview.transactions));
  const [confirming, setConfirming] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [confirmedOutcome, setConfirmedOutcome] = useState<{ importedCount: number; duplicateCount: number; rulesAutoAllocated: number; exceptions: { description: string }[] } | null>(null);

  const { balanceReconciliation, runningBalanceIssues, transactionCount, invalidValueIssues } = preview.validation;

  function updateTransactions(next: ParsedBankTransaction[]) {
    setTransactions(next);
    if (typeof window === "undefined") return;
    try {
      window.sessionStorage.setItem(transactionsStorageKey(companyId, preview.batchId), JSON.stringify(next));
    } catch {
      // sessionStorage full/unavailable — editing still works, just without draft persistence
    }
  }
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      if (typeof window !== "undefined") window.sessionStorage.removeItem(transactionsStorageKey(companyId, preview.batchId));
      onConfirmed?.();
      router.refresh();
    } catch {
  ...
  if (confirmedOutcome) {
    return (
      <ImportOutcomeSummary
        ...
        exceptionCount={preview.exceptions.length + (confirmedOutcome.exceptions?.length ?? 0)}
    ...
    <Card>
      ...
      <CardContent className="flex flex-col gap-4 pt-0">
        ...
        <BatchEntryGrid
          columns={COLUMNS}
          rows={transactions}
          onRowsChange={updateTransactions}
          createEmptyRow={() => ({
