    ...
    <div className="rounded-vf-md border border-vf-paper-border p-4">
      ...
      {activeForm === "transfer" && (
        <div className="mt-3 flex flex-wrap items-center gap-2 border-t border-vf-paper-border pt-3">
          ...
          <Input placeholder="Reason (recorded to audit trail)" value={transferReason} onChange={(e) => setTransferReason(e.target.value)} className="w-64" />
          <Button variant="primary" size="sm" disabled={loading} onClick={() => submit("transfer", { location: description, reason: transferReason.trim() || undefined })}>
            Confirm Transfer
          </Button>
          <Button variant="subtle" size="sm" onClick={() => setActiveForm(null)}>Cancel</Button>
        </div>
      ...
      {activeForm === "revalue" && (
        <div className="mt-3 flex flex-wrap items-center gap-2 border-t border-vf-paper-border pt-3">
          ...
          <Input placeholder="Reason (recorded to audit trail)" value={revalueReason} onChange={(e) => setRevalueReason(e.target.value)} className="w-64" />
          <Button variant="primary" size="sm" disabled={loading || !delta} onClick={() => submit("revalue", { delta: Number(delta), reason: revalueReason.trim() || undefined })}>
            Confirm
          </Button>
          <Button variant="subtle" size="sm" onClick={() => setActiveForm(null)}>Cancel</Button>
        </div>
