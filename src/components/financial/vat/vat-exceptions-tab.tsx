...
const EXCEPTION_LABEL: Record<VatExceptionType, string> = {
  ...
  IncorrectVatCode: "Incorrect VAT Code",
  UnexpectedVatPercentage: "Unexpected VAT Percentage",
  LargeVatAdjustment: "Large VAT Adjustment",
  DuplicateVatClaim: "Duplicate VAT Claim",
  VatRateConflict: "VAT Rate Conflict",
  CrossPeriodVat: "Cross-Period VAT",
};

const STATUS_TONE: Record<VatExceptionStatus, "warn" | "good" | "muted"> = { Open: "warn", Resolved: "good", Dismissed: "muted" };

function ExceptionCard({ exception, companyId, previewMode }: { exception: VatException; companyId: string; previewMode: boolean }) {
  ...
  async function submit(status: "Resolved" | "Dismissed") {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/companies/${companyId}/vat-exceptions/${exception.id}`, {
        method: "PATCH",
      ...
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-vf-ink-faint">{exception.documentType} #{exception.documentId}</span>
          </div>
...
export function VatExceptionsTab({ companyId, exceptions, previewMode }: { companyId: string; exceptions: VatException[]; previewMode: boolean }) {
    ...
    return (
      <Card>
        <CardContent className="pt-6">
          <EmptyState title="No VAT exceptions." description="Run VAT Intelligence from the Intelligence tab to scan for issues." />
        </CardContent>
      </Card>
    );
  }
