...
import { supplierOptions } from "@/lib/account-picker-options";
...
const STATUS_OPTIONS: (SupplierPaymentStatus | "All")[] = ["All", "Draft", "Approved", "Posted", "Cancelled"];
...
function PaymentFormPanel({ companyId, suppliers, bankAccounts, onDone, onCancel }: { companyId: string; suppliers: Supplier[]; bankAccounts: BankAccount[]; onDone: () => void; onCancel: () => void }) {
  ...
  async function submit() {
    ...
    try {
      const res = await fetch(`/api/companies/${companyId}/purchasing/payments`, {
        ...
        body: JSON.stringify({ supplierId, paymentDate, amount: Number(amount) || 0, reference, bankAccountId: bankAccountId || null }),
      });
    ...
    <div className="rounded-vf-md border border-vf-paper-border p-4">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-4">
        <Field label="Supplier" htmlFor="sp-supplier" required>
          ...
          <Combobox aria-label="Supplier" value={supplierId || null} options={activeSupplierOptions} placeholder="Search supplier…" onCommit={(val) => setSupplierId(val ?? 0)} />
        </Field>
        ...
        {/* Finding #117 — see receipts-tab.tsx's identical note. */}
        <Field label="Bank Account" htmlFor="sp-bank">
          <Select id="sp-bank" value={bankAccountId} onChange={(e) => setBankAccountId(e.target.value ? Number(e.target.value) : "")}>
...
export function PaymentsTab({
  ...
  const [error, setError] = useState<string | null>(null);
  // Finding #031 — removing one misapplied allocation line.
  const unallocateConfirm = useConfirmTarget<number>();
  ...
  const filtered = payments.filter((p) => {
    ...
    if (!term) return true;
    return p.paymentNumber.toLowerCase().includes(term) || supplierName(p.supplierId).toLowerCase().includes(term);
  });
  ...
  async function removeAllocation(allocationId: number, paymentId: number) {
    setUnallocating(true);
    setError(null);
    try {
      const res = await fetch(`${base}/${paymentId}/allocations?allocationId=${allocationId}`, { method: "DELETE" });
      const data = await res.json();
      if (!res.ok) {
        setError(data.error ?? `Request failed (${res.status})`);
        return;
      }
      unallocateConfirm.cancel();
      router.refresh();
    } catch {
      setError("Couldn't reach the API. Check the dev server is running.");
    } finally {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {showForm && <PaymentFormPanel companyId={companyId} suppliers={suppliers} bankAccounts={bankAccounts} onDone={() => { setShowForm(false); router.refresh(); }} onCancel={() => setShowForm(false)} />}
                                ...
                                <tr key={a.id} className="border-b border-vf-paper-border/60">
                                  ...
                                  <td className="py-1 text-right">
                                    {unallocateConfirm.isConfirming(a.id) ? (
                                      <ConfirmActionRow
                                        message="Remove this allocation?"
                                        confirmLabel="Confirm"
                                        confirmingLabel="Removing…"
                                        loading={unallocating}
                                        tone="danger"
                                        size="sm"
                                        onConfirm={() => removeAllocation(a.id, p.id)}
                                        onCancel={unallocateConfirm.cancel}
                                      />
                                    ) : (
                                      <Button variant="subtle" size="sm" disabled={previewMode} title={disabledTitle} onClick={() => unallocateConfirm.request(a.id)}>
                                        Remove
                                      </Button>
                                    )}
                                  </td>
