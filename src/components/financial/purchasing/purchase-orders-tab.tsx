...
function CreateBillPanel({
  ...
  async function submit() {
    ...
    try {
      const res = await fetch(`/api/companies/${companyId}/purchasing/bills`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ orderId: order.id, invoiceNumber, invoiceDate, vatTreatmentCode }),
      });
      const data = await res.json();
      if (!res.ok) {
        setError(data.error ?? `Request failed (${res.status})`);
  ...
  return (
    <div className="flex flex-wrap items-end gap-2 rounded-vf-sm border border-vf-paper-border p-3">
      ...
      <div className="w-44">
        {/* Finding #222 — previously always silently sent whichever VAT
            treatment happened to be first in the list, with no control.
            Only consumed by the legacy fallback path — ignored when every
            order line already carries its own vatCode. */}
        <Field label="VAT Treatment" htmlFor={`po-bill-vat-${order.id}`}>
          <Select id={`po-bill-vat-${order.id}`} value={vatTreatmentCode} disabled={everyLineDimensioned} onChange={(e) => setVatTreatmentCode(e.target.value)}>
            {vatTreatments.map((v) => (
...
export function PurchaseOrdersTab({
  ...
  async function runAction(id: number, action: "submit" | "approve" | "reject" | "cancel" | "clone-as-draft") {
    setLoadingId(id);
    setError(null);
    try {
                  ...
                  <TableRow>
                    ...
                    <TableCell className="text-right">
                      <div className="flex flex-wrap justify-end gap-1.5">
                        ...
                        {/* Finding #056 — a PartiallyReceived order can
                            now be billed for whatever's been received but
                            not yet billed, not just once fully Received. */}
                        {(o.status === "Received" || o.status === "PartiallyReceived") && o.lines.some((l) => l.receivedQuantity > l.billedQuantity) && (
                          <Button variant="primary" size="sm" disabled={previewMode} title={disabledTitle} onClick={() => setBillingId(billingId === o.id ? null : o.id)}>
                            Create Bill
                          </Button>
                        )}
                        {o.status === "Rejected" && (
                          /* Finding #166 — see quotations-tab.tsx's identical note. */
                          <Button variant="subtle" size="sm" disabled={previewMode || loadingId === o.id} title={disabledTitle} onClick={() => runAction(o.id, "clone-as-draft")}>
                            Clone as Draft
                          </Button>
                        )}
                      </div>
                  ...
                  {billingId === o.id && (
                    <TableRow>
                      <TableCell colSpan={7} className="bg-vf-paper-alt/40">
                        <CreateBillPanel companyId={companyId} order={o} vatTreatments={vatTreatments} defaultVatCode={defaultVatCode} onDone={() => { setBillingId(null); router.refresh(); }} onCancel={() => setBillingId(null)} />
                      </TableCell>
                    </TableRow>
                  )}
                  {editingId === o.id && (
                    ...
                    <TableRow>
                      <TableCell colSpan={7} className="bg-vf-paper-alt/40">
                        ...
                        <div className="mt-3 flex flex-col gap-3">
                          ...
                          <DocumentsPanel companyId={companyId} entityType="PurchaseOrder" entityId={o.id} previewMode={previewMode} />
                        </div>
                      </TableCell>
                    </TableRow>
                  )}
