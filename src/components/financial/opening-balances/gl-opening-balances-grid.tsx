...
export function GlOpeningBalancesGrid({
  ...
  const [initialRows] = useState<GridRow[]>(() => buildInitialRows(chartOfAccounts, bankAccounts, entries));
  const [rows, setRows] = useState<GridRow[]>(initialRows);
  const [reason, setReason] = useState("");
  // Master Implementation Tracker — Epic E1, Root Cause RC-1, Finding
  // #184. One balance date for the whole batch (opening balances
  // represent a single "as at" moment — the company's go-live date, not
  // whenever each row happened to be typed in), seeded from an existing
  // draft entry's own `balanceDate` if one was already set (e.g. by a
  // prior save this session), otherwise today as a starting point the
  // accountant can change before saving. This is what makes the eventual
  // posted journal's date correct instead of defaulting to "today the
  // journal happened to be posted."
  const [balanceDate, setBalanceDate] = useState(() => entries.find((e) => e.balanceDate)?.balanceDate ?? new Date().toISOString().slice(0, 10));
  ...
  useUnsavedChangesWarning(rows.some((row, i) => row.debit !== initialRows[i]?.debit || row.credit !== initialRows[i]?.credit));
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <Field label="Balance date" htmlFor="ob-grid-balance-date">
          <Input id="ob-grid-balance-date" type="date" value={balanceDate} onChange={(e) => setBalanceDate(e.target.value)} disabled={loading} />
          <p className="mt-1 text-xs text-vf-ink-faint">The date these balances are effective as at — usually your go-live date, not today. This is the date the posted journal will carry.</p>
        </Field>
