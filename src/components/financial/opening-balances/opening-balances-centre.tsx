...
export function OpeningBalancesCentre({
  ...
  // Master Implementation Tracker — Epic E1, Finding #212 (RC-3). Remove
  // used to fire immediately on click. The `window.prompt` for a
  // go-live reason is replaced by an inline field in the confirm row.
  const confirmDelete = useConfirmTarget<number>();
  ...
  async function handleDelete(entry: OpeningBalanceEntry) {
    setError(null);
    if (governance.reasonRequired && !deleteReason.trim()) {
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                {category === "Customer" ? (
                  ...
                  <Field label="Supplier" htmlFor="ob-target">
                    <Select id="ob-target" value={targetId} onChange={(e) => setTargetId(e.target.value)}>
                      <option value="">Select…</option>
                      {/* Phase 38 — a new opening balance entry must never
                       * target an Inactive supplier. */}
            ...
            <TableBody>
              {entries.map((entry) => (
                <TableRow key={entry.id}>
                  ...
                  <TableCell>
                    {entry.status === "draft" &&
                      (confirmDelete.isConfirming(entry.id) ? (
                        <div className="flex flex-col items-end gap-1.5">
                          {governance.reasonRequired && (
                            <Input
                              ...
                              aria-label="Reason for removing this opening balance"
                            />
                          )}
                          <ConfirmActionRow
                            message="Remove this opening balance entry?"
                            tone="danger"
                        ...
                        <Button variant="subtle" size="sm" disabled={loading} onClick={() => confirmDelete.request(entry.id)}>
                          Remove
                        </Button>
                      ))}
                  </TableCell>
