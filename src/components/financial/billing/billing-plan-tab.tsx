...
export function BillingPlanTab({
  ...
  // Finding #127 (RC-3) — mirrors the established `ConfirmActionRow`/
  // `useConfirmTarget` pattern.
  const cancelConfirm = useConfirmTarget<true>();
    ...
    <div className="flex flex-col gap-6">
      <div className="flex flex-wrap items-center justify-between gap-4 rounded-xl border border-vf-paper-border p-4">
        ...
        <div className="flex gap-2">
          ...
          ) : cancelConfirm.isConfirming(true) ? (
            <ConfirmActionRow
              message="Cancel this plan at the end of the current billing period?"
              loading={loading}
              tone="danger"
              onConfirm={() => callAction("cancel", { cancelImmediately: false })}
              onCancel={cancelConfirm.cancel}
            />
          ) : (
            <Button variant="subtle" size="sm" onClick={() => cancelConfirm.request(true)} disabled={loading || previewMode || subscription.status === "cancelled"} title={previewMode ? "Available once a production Supabase project is connected" : undefined}>
              Cancel Plan
            </Button>
          )}
        </div>
