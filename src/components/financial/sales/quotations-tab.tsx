...
import { customerOptions } from "@/lib/account-picker-options";
...
const BLANK_LINE: EditableLine = { description: "", quantity: "1", unitPrice: "", vatCode: null };
...
function QuotationFormPanel({
  ...
  // Master Implementation Tracker — Epic E11, Finding #198 (RC-9).
  const [initialSnapshot] = useState(() => JSON.stringify({ customerId, quotationDate, expiryDate, lines }));
  useUnsavedChangesWarning(JSON.stringify({ customerId, quotationDate, expiryDate, lines }) !== initialSnapshot);
    ...
    try {
      const res = await fetch(`/api/companies/${companyId}/sales/quotations`, {
        ...
        body: JSON.stringify({
          ...
          lines: lines.map((l) => ({ description: l.description, quantity: Number(l.quantity) || 0, unitPrice: Number(l.unitPrice) || 0, vatCode: l.vatCode })),
        }),
      });
    ...
    <div className="rounded-vf-md border border-vf-paper-border p-4">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        <Field label="Customer" htmlFor="qt-customer" required>
          {/* Finding #200 — see deliveries-tab.tsx's identical note.
              Finding #167 — searchable Combobox instead of a plain Select. */}
      ...
      <div className="mt-4 flex flex-col gap-2">
        ...
        {lines.map((line, i) => (
          <div key={i} className="flex flex-wrap items-center gap-2">
            ...
            <div className="w-40">
              {/* Finding #112 — Quotations had no VAT concept at all. */}
...
/** Finding #055 — mirrors `sales-orders-tab.tsx`'s own
 * `EditOrderLinesPanel`: only ever rendered for a Draft quotation, reusing
 * `quotation.lines` already embedded in the list response. */
function EditQuotationLinesPanel({ companyId, quotation, vatTreatments, onDone, onCancel }: { companyId: string; quotation: Quotation; vatTreatments: VatTreatment[]; onDone: () => void; onCancel: () => void }) {
  const [lines, setLines] = useState<EditableLine[]>(() =>
    quotation.lines.length > 0
      ? quotation.lines.map((l) => ({ description: l.description, quantity: String(l.quantity), unitPrice: String(l.unitPrice), vatCode: l.vatCode }))
      : [{ ...BLANK_LINE }],
  );
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  ...
  async function save() {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/companies/${companyId}/sales/quotations/${quotation.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          action: "update-lines",
  ...
  return (
    <div className="rounded-vf-md border border-vf-paper-border p-4">
      <p className="mb-3 text-sm font-semibold text-vf-ink">Editing {quotation.quotationNumber}</p>
...
export function QuotationsTab({
  ...
  async function runAction(id: number, action: "send" | "accept" | "reject" | "expire" | "clone-as-draft") {
    setLoadingId(id);
    setError(null);
    try {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {showForm && (
        <QuotationFormPanel companyId={companyId} customers={customers} vatTreatments={vatTreatments} onDone={() => { setShowForm(false); router.refresh(); }} onCancel={() => setShowForm(false)} />
      )}
                  ...
                  <TableRow>
                    ...
                    <TableCell className="text-right">
                      <div className="flex flex-wrap justify-end gap-1.5">
                        {q.status === "Draft" && (
                          <>
                            {/* Finding #055 — Draft previously had no
                                Edit or Reject exposed, even though the
                                backend already supported Reject
                                (Draft -> Rejected) and now supports line
                                editing too. */}
                            <Button variant="subtle" size="sm" disabled={previewMode || loadingId === q.id} title={disabledTitle} onClick={() => setEditingId(editingId === q.id ? null : q.id)}>
                              Edit
                            </Button>
                        ...
                        {q.status === "Rejected" && (
                          /* Finding #166 — a Rejected quotation used to be a
                             dead end. Rather than reopening the terminal
                             Rejected document, this clones a new Draft with
                             the same customer/lines to revise and re-send. */
                          <Button variant="subtle" size="sm" disabled={previewMode || loadingId === q.id} title={disabledTitle} onClick={() => runAction(q.id, "clone-as-draft")}>
                            Clone as Draft
                          </Button>
                        )}
                      </div>
                  ...
                  {editingId === q.id && (
                    <TableRow>
                      <TableCell colSpan={8} className="bg-vf-paper-alt/40">
                        <EditQuotationLinesPanel companyId={companyId} quotation={q} vatTreatments={vatTreatments} onDone={() => { setEditingId(null); router.refresh(); }} onCancel={() => setEditingId(null)} />
                      </TableCell>
                    </TableRow>
                  )}
                  {isExpanded && (
                    <TableRow>
                      <TableCell colSpan={8} className="bg-vf-paper-alt/40">
                        ...
                        <div className="mt-3 flex flex-col gap-3">
                          ...
                          <DocumentsPanel companyId={companyId} entityType="Quotation" entityId={q.id} previewMode={previewMode} />
                        </div>
                      </TableCell>
                    </TableRow>
                  )}
