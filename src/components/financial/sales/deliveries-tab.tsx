...
const STATUS_OPTIONS: (DeliveryStatus | "All")[] = ["All", "Draft", "Delivered", "Cancelled"];

type EditableLine = { orderLineId: number | null; description: string; quantity: string; max: number | null; stockItemId: number | null };
...
function DeliveryFormPanel({
  ...
  const [lines, setLines] = useState<EditableLine[]>([{ orderLineId: null, description: "", quantity: "1", max: null, stockItemId: null }]);
  ...
  function selectOrder(value: string) {
    if (!value) {
      setOrderId("");
      setLines([{ orderLineId: null, description: "", quantity: "1", max: null, stockItemId: null }]);
      return;
    }
    ...
    setLines(
      order.lines
        ...
        .map((l) => ({ orderLineId: l.id, description: l.description, quantity: String(l.quantity - l.deliveredQuantity), max: l.quantity - l.deliveredQuantity, stockItemId: l.stockItemId })),
    );
  }
    ...
    try {
      const res = await fetch(`/api/companies/${companyId}/sales/deliveries`, {
        ...
        body: JSON.stringify({
          ...
          deliveryDate,
          lines: lines.map((l) => ({ orderLineId: l.orderLineId, description: l.description, quantity: Number(l.quantity) || 0, stockItemId: l.stockItemId })),
        }),
      });
    ...
    <div className="rounded-vf-md border border-vf-paper-border p-4">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        ...
        <Field label="Customer" htmlFor="dl-customer" required>
          {/* Finding #200 — a deactivated customer no longer appears as
              a pick for a NEW document; an existing delivery already
              tied to one still displays/edits fine elsewhere. Finding
              #167 — searchable Combobox instead of a plain Select. */}
          <Combobox aria-label="Customer" value={customerId || null} disabled={orderId !== ""} options={activeCustomerOptions} placeholder="Search customer…" onCommit={(val) => setCustomerId(val ?? 0)} />
        </Field>
      ...
      <div className="mt-4 flex flex-col gap-2">
        ...
        {orderId === "" && (
          <Button variant="subtle" size="sm" className="w-fit" onClick={() => setLines((prev) => [...prev, { orderLineId: null, description: "", quantity: "1", max: null, stockItemId: null }])}>
...
export function DeliveriesTab({
  ...
  const filtered = deliveries.filter((d) => {
    ...
    if (!term) return true;
    return d.deliveryNumber.toLowerCase().includes(term) || customerName(d.customerId).toLowerCase().includes(term);
  });
                  ...
                  <TableRow>
                    ...
                    <TableCell className="text-right">
                      <div className="flex flex-wrap justify-end gap-1.5">
                        {d.status === "Delivered" && (
                          <>
                            {cancelConfirm.isConfirming(d.id) ? (
                              <ConfirmActionRow message="Cancel this delivery?" confirmLabel="Confirm" confirmingLabel="Cancelling…" loading={loadingId === d.id} tone="danger" size="sm" onConfirm={() => cancelDelivery(d.id)} onCancel={cancelConfirm.cancel} />
                            ) : (
                              <Button variant="subtle" size="sm" disabled={previewMode || loadingId === d.id} title={disabledTitle} onClick={() => cancelConfirm.request(d.id)}>
                                Cancel
                              </Button>
                            )}
                    ...
                    <TableRow>
                      <TableCell colSpan={7} className="bg-vf-paper-alt/40">
                        ...
                        <div className="mt-3 flex flex-col gap-3">
                          ...
                          <DocumentsPanel companyId={companyId} entityType="Delivery" entityId={d.id} previewMode={previewMode} />
                        </div>
                      </TableCell>
                    </TableRow>
                  )}
