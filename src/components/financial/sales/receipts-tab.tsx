...
function ReceiptFormPanel({ companyId, customers, bankAccounts, onDone, onCancel }: { companyId: string; customers: Customer[]; bankAccounts: BankAccount[]; onDone: () => void; onCancel: () => void }) {
  ...
  async function submit() {
    ...
    try {
      const res = await fetch(`/api/companies/${companyId}/sales/receipts`, {
        ...
        body: JSON.stringify({ customerId, receiptDate, amount: Number(amount) || 0, reference, bankAccountId: bankAccountId || null }),
      });
  ...
  return (
    <div className="rounded-vf-md border border-vf-paper-border p-4">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-4">
        ...
        {/* Finding #116 — bankAccountId was already a real, persisted
            column with no UI ever setting it, always sent null — a real
            downstream gap for bank reconciliation. */}
        <Field label="Bank Account" htmlFor="rc-bank">
          <Select id="rc-bank" value={bankAccountId} onChange={(e) => setBankAccountId(e.target.value ? Number(e.target.value) : "")}>
...
export function ReceiptsTab({
  ...
  const filtered = receipts.filter((r) => {
    ...
    if (!term) return true;
    return r.receiptNumber.toLowerCase().includes(term) || customerName(r.customerId).toLowerCase().includes(term);
  });
  ...
  async function removeAllocation(allocationId: number, receiptId: number) {
    setUnallocating(true);
    setError(null);
    try {
      const res = await fetch(`${base}/${receiptId}/allocations?allocationId=${allocationId}`, { method: "DELETE" });
      const data = await res.json();
      if (!res.ok) {
        setError(data.error ?? `Request failed (${res.status})`);
        return;
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {showForm && <ReceiptFormPanel companyId={companyId} customers={customers} bankAccounts={bankAccounts} onDone={() => { setShowForm(false); router.refresh(); }} onCancel={() => setShowForm(false)} />}
                                  ...
                                  <td className="py-1 text-right">
                                    {unallocateConfirm.isConfirming(a.id) ? (
                                      <ConfirmActionRow
                                        ...
                                        tone="danger"
                                        size="sm"
                                        onConfirm={() => removeAllocation(a.id, r.id)}
