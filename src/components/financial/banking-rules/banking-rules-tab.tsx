...
/** Phase 51 — pure and exported so it's directly unit-testable without
 * rendering the page (100+ rules, mount-heavy). Matches by rule
 * name/description, any condition's value, or any action's target
 * (`targetText` for GL/free-text actions, `targetId` for
 * merchant/supplier/customer id-based actions) — the four fields the
 * requirement names. Case-insensitive substring, same convention every
 * other search box in this app already uses. */
export function ruleMatchesSearch(rule: BankingRule, query: string): boolean {
  const q = query.trim().toLowerCase();
  if (!q) return true;
  if (rule.name.toLowerCase().includes(q)) return true;
  if (rule.description.toLowerCase().includes(q)) return true;
  if (rule.conditions.some((c) => c.value.toLowerCase().includes(q))) return true;
  if (rule.actions.some((a) => (a.targetText ?? "").toLowerCase().includes(q) || (a.targetId !== null && String(a.targetId).includes(q)))) return true;
  return false;
}

// Finding #006 — of the 9 domains `RULE_DOMAINS` offers, only these have
// a real evaluator wired up anywhere in `src/server` today (Banking via
// `rule-processing-service.ts`, VAT via `vat-rule-service.ts` +
// `vat-exception-scan-service.ts`). The other 7 are genuinely
// UI-selectable and persist correctly — deliberate incremental rollout,
// not a defect — but a rule saved there will never fire until its own
// domain gets an evaluator, so the create form discloses this rather
// than silently letting a user build a rule that can never run.
const LIVE_DOMAINS: RuleDomain[] = ["Banking", "VAT"];
...
function ActionEditor({ actions, onChange, actionTypes, domain }: { actions: EditableAction[]; onChange: (a: EditableAction[]) => void; actionTypes: string[]; domain: RuleDomain }) {
  function update(i: number, patch: Partial<EditableAction>) {
    onChange(actions.map((a, idx) => (idx === i ? { ...a, ...patch } : a)));
  }
  // QA finding — a Banking-domain "Flag for Review" action's value is
  // free-text only (rule-processing-service.ts always raises the
  // UnbalancedAllocation exception type regardless of what's typed here);
  // only a VAT-domain rule's `targetText` is actually matched against a
  // real VatExceptionType (vat-rule-service.ts). The placeholder must not
  // claim a capability Banking rules don't have.
  const targetValuePlaceholder = domain === "VAT" ? "Value (GL code, VAT code, exception type…)" : "Value (GL code, VAT code, or a note for Flag for Review)";
  return (
    <div className="flex flex-col gap-2">
      ...
      {actions.map((a, i) => (
        <div key={i} className="flex flex-wrap items-center gap-2 rounded-vf-sm bg-vf-paper-alt/60 p-2 text-sm">
          ...
          {ACTION_TAKES_ID.includes(a.actionType) ? (
            ...
            <div className="w-56">
              <Input placeholder={targetValuePlaceholder} value={a.targetText} aria-label={`Target value for action ${i + 1}`} onChange={(e) => update(i, { targetText: e.target.value })} />
            </div>
          )}
...
function RuleCard({
  ...
  selected: boolean;
  onToggleSelect: (ruleId: number) => void;
}) {
  const router = useRouter();
  const initialConditions = rule.conditions.map((c) => ({ field: c.field, operator: c.operator, value: c.value, value2: c.value2 ?? "" }));
  const initialActions = rule.actions.map((a) => ({ actionType: a.actionType, targetId: a.targetId !== null ? String(a.targetId) : "", targetText: a.targetText ?? "" }));
  const [name, setName] = useState(rule.name);
  const [description, setDescription] = useState(rule.description);
  const [priority, setPriority] = useState(String(rule.priority));
  const [conditions, setConditions] = useState<EditableCondition[]>(initialConditions);
  const [actions, setActions] = useState<EditableAction[]>(initialActions);
  // Finding #091 — Save Changes only enables once something has actually
  // changed, instead of always being clickable.
  const dirty =
    name !== rule.name ||
  ...
  return (
    <div className="rounded-vf-md border border-vf-paper-border p-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          <div className="flex items-center gap-2">
            <input type="checkbox" checked={selected} onChange={() => onToggleSelect(rule.id)} aria-label={`Select rule ${rule.name}`} />
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {deleteConfirm.isConfirming(true) ? (
            ...
            <Button variant="subtle" size="sm" className="text-vf-danger" disabled={previewMode || loading} title={disabledTitle} onClick={() => deleteConfirm.request(true)}>
              Delete
            </Button>
          )}
        </div>
      ...
      <div className="mt-3 grid grid-cols-1 gap-3 lg:grid-cols-2">
        ...
        <ActionEditor actions={actions} onChange={setActions} actionTypes={DOMAIN_ACTION_TYPES[rule.domain]} domain={rule.domain} />
      </div>
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2">
        <Button
          variant="subtle"
          size="sm"
          disabled={previewMode || loading || !dirty}
          title={!dirty ? "No changes to save" : disabledTitle}
          onClick={() =>
            patch({
              name,
              description,
...
function NewRuleForm({
  ...
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  // Finding #092 — a non-mutating check before creation, mirroring
  // Transaction Explorer's own "Set Rule" checkbox flow
  // (`transaction-explorer.tsx::checkDuplicateRule`), so an exact
  // duplicate surfaces as a confirmable warning instead of silently
  // creating a second identical rule.
  ...
  async function submit() {
    ...
    try {
      const res = await fetch(`/api/companies/${companyId}/banking-rules/check-duplicate`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ domain, ruleType, conditions: toApiConditions(conditions), actions: toApiActions(actions) }),
      });
      if (res.ok) {
        const data = await res.json();
        if (data.duplicate?.name) {
          setDuplicateOf(data.duplicate.name);
          return;
        }
      }
    } catch {
      // Couldn't reach the duplicate-check endpoint — fall through and
      // create normally rather than blocking rule creation on it.
    } finally {
      setLoading(false);
    }
    await create();
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        ...
        <Field label="Name" htmlFor="new-rule-name" required>
          <Input
            ...
            placeholder="e.g. Netherfield Freight -> Distribution"
          />
        </Field>
      </div>
      {!LIVE_DOMAINS.includes(domain) && (
        <p className="mt-2 text-xs text-vf-warning">
          {domain} rules are saved but not yet evaluated automatically — only Banking and VAT rules run today.
        </p>
      )}
      <div className="mt-3 grid grid-cols-1 gap-3 lg:grid-cols-2">
        <ConditionEditor conditions={conditions} onChange={(c) => { setConditions(c); setDuplicateOf(null); }} fields={DOMAIN_CONDITION_FIELDS[domain]} />
        <ActionEditor actions={actions} onChange={(a) => { setActions(a); setDuplicateOf(null); }} actionTypes={DOMAIN_ACTION_TYPES[domain]} domain={domain} />
      </div>
      {duplicateOf && (
        <p className="mt-2 text-xs text-vf-warning">
          This looks identical to the existing rule &ldquo;{duplicateOf}&rdquo; — click Create Rule again to create it anyway.
        </p>
      )}
      <div className="mt-3 flex gap-2">
        <Button variant="primary" size="sm" disabled={loading || !name.trim()} onClick={submit}>
          {duplicateOf ? "Create Anyway" : "Create Rule"}
        </Button>
...
export function BankingRulesTab({ companyId, rules, previewMode }: { companyId: string; rules: BankingRule[]; previewMode: boolean }) {
  ...
  // Phase 51 — the rule count has grown past 100 with only a domain
  // dropdown to narrow it (see the Phase 50 forensic report). A plain
  // client-side text/status filter over the rules this page already has
  // — no new endpoint, no redesign, no pagination.
  ...
  // + bulk-action-bar pattern Transaction Explorer already established
  // (`transaction-bulk-action-bar.tsx`). Loops the existing single-rule
  // PATCH/DELETE endpoints client-side rather than adding a new bulk API
  // route — the smallest correct fix for a rule count that's never more
  // than a page or two of cards.
  const [selectedIds, setSelectedIds] = useState<Set<number>>(new Set());
  const [bulkWorking, setBulkWorking] = useState(false);
  const bulkDeleteConfirm = useConfirmTarget<true>();
  const apiBase = `/api/companies/${companyId}/banking-rules`;
  const disabledTitle = previewMode ? "Available once a production Supabase project is connected" : undefined;

  function toggleSelect(ruleId: number) {
    setSelectedIds((s) => {
  ...
  async function bulkSetActive(isActive: boolean) {
    ...
    try {
      await Promise.all(
        [...selectedIds].map((id) =>
          fetch(`${apiBase}/${id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ isActive }) }),
        ),
      );
  ...
  async function bulkDelete() {
    setBulkWorking(true);
    try {
      await Promise.all([...selectedIds].map((id) => fetch(`${apiBase}/${id}`, { method: "DELETE" })));
      setSelectedIds(new Set());
      bulkDeleteConfirm.cancel();
      router.refresh();
    } finally {
      setBulkWorking(false);
    }
  ...
  useEffect(() => {
    ...
    // Finding #090 — this used to be hardcoded to "Banking" regardless of
    // which domain the user is actually viewing; conflicts only ever
    // showed up for Banking rules even while looking at VAT/Sales/etc.
    // `getRuleConflicts` takes one domain per call, so "All" falls back
    // to Banking (the domain conflicts matter most for today).
    const conflictDomain = domainFilter === "All" ? "Banking" : domainFilter;
    fetch(`${apiBase}/conflicts?domain=${conflictDomain}`)
      .then((res) => (res.ok ? res.json() : { conflicts: [] }))
      .then((data) => setConflicts(data.conflicts ?? []))
      .catch(() => setConflicts([]));
  }, [apiBase, previewMode, rules, domainFilter]);
  ...
  return (
    <div className="flex flex-col gap-3">
      <p className="text-sm text-vf-ink-faint">
        ...
        Rules run on import and via &ldquo;Run Rule Engine Now&rdquo; below — unattended scheduled runs require a
        configured cron job, not yet connected.
      </p>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <div className="w-36">
          <Select aria-label="Filter by status" value={statusFilter} onChange={(e) => setStatusFilter(e.target.value as "All" | "Active" | "Inactive")}>
        ...
        <div className="min-w-[220px] flex-1">
          <Input
            ...
            placeholder="Search name, description, condition, or target…"
            value={searchQuery}
        ...
        {/* Phase 32 — served from a route so the header source
         * (`banking-rule-service.ts`) never has to be imported into this
         * client component. */}
        <Button variant="subtle" size="sm" disabled={previewMode} title={disabledTitle} onClick={() => window.open(`${apiBase}/import-template`, "_blank")}>
      ...
      <p className="text-xs text-vf-ink-faint">
        Download the template, complete it, and upload it here. One row per condition —{" "}
        <span className="font-medium text-vf-ink-soft">Rule Type, Name, Condition Field, Operator, and Value</span> are required.
        {" "}Priority: whole number (leave blank for the default of 100).
      </p>
      ...
      {selectedIds.size > 0 &&
        (bulkDeleteConfirm.isConfirming(true) ? (
          <ConfirmActionRow
            message={`Delete ${selectedIds.size} selected rule(s)?`}
            confirmLabel="Confirm Delete"
            confirmingLabel="Deleting…"
            loading={bulkWorking}
            tone="danger"
            onConfirm={bulkDelete}
            onCancel={bulkDeleteConfirm.cancel}
          />
        ) : (
          <div className="flex flex-wrap items-center gap-2 rounded-vf-md border border-vf-paper-border bg-vf-paper-alt/60 px-3 py-2 text-sm">
            ...
            <Button variant="subtle" size="sm" disabled={previewMode || bulkWorking} title={disabledTitle} onClick={() => bulkSetActive(true)}>
              Enable
            </Button>
            <Button variant="subtle" size="sm" disabled={previewMode || bulkWorking} title={disabledTitle} onClick={() => bulkSetActive(false)}>
              Disable
            </Button>
            <Button variant="subtle" size="sm" className="text-vf-danger" disabled={previewMode || bulkWorking} title={disabledTitle} onClick={() => bulkDeleteConfirm.request(true)}>
              Delete
            </Button>
      ...
      {(() => {
        const visibleRules = rules
          ...
          .filter((r) => statusFilter === "All" || (statusFilter === "Active") === r.isActive)
          .filter((r) => ruleMatchesSearch(r, searchQuery));
        const noRulesAtAll = rules.length === 0;
        return visibleRules.length === 0 ? (
          <EmptyState
            icon={<IconSliders className="h-5 w-5" />}
            title={noRulesAtAll ? "No automation rules yet." : "No rules match your search/filters."}
            description={
              noRulesAtAll
                ? "Add your first rule above, or use Learn Rule from a transaction in Transaction Explorer."
                : "Try a different search term, or clear the domain/status filters."
            }
          />
        ) : (
          visibleRules.map((rule) => (
            <RuleCard
              ...
              conflictReasons={conflictReasonsFor(rule.id)}
