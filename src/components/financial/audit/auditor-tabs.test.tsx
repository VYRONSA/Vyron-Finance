...
import { MOCK_AUDIT_ENGAGEMENTS, MOCK_AUDIT_AREAS, MOCK_AUDIT_RISK_REGISTER, MOCK_AUDIT_TEAM_ASSIGNMENTS, MOCK_AUDIT_FINDINGS, MOCK_AUDIT_WORKING_PAPERS, MOCK_AUDIT_QUERIES, MOCK_COMPANY_MEMBERS } from "@/lib/mock/audit-data";
...
function renderTabs(previewMode = true) {
  ...
  return render(
    <AuditorTabs
      ...
      companyMembers={MOCK_COMPANY_MEMBERS}
...
describe("AuditorTabs", () => {
  ...
  it("renders a Lead Schedule working paper as a formatted table, not raw JSON, when expanded", () => {
    renderTabs();
    fireEvent.click(screen.getByRole("button", { name: "Working Papers" }));
    fireEvent.click(screen.getAllByRole("button", { name: "View" })[0]);
    expect(screen.queryByText(/^\{/)).not.toBeInTheDocument();
    const firstLeadLine = MOCK_AUDIT_WORKING_PAPERS[0].content.sections as { lines: { accountCode: string }[] }[];
    expect(screen.getByText(firstLeadLine[0].lines[0].accountCode)).toBeInTheDocument();
  });
