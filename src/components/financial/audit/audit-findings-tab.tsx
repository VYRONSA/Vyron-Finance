...
function FindingCard({
  ...
  // Finding #158 — a real user picker (`CommunicationRecipient.type`
  // already has a first-class `"User"` variant this platform never used)
  // instead of a free-text email. `id` stays `null` for a User recipient:
  // this platform's real users are Supabase Auth users with UUID ids,
  // and `CommunicationRecipient.id` is typed `number | null` — delivery
  // itself only ever reads `.address`, never `.id`, so this loses nothing
  // functional. Keeps an "Other" escape hatch since a query recipient
  // isn't always a registered company user.
  const [selectedUserId, setSelectedUserId] = useState(companyMembers[0]?.userId ?? OTHER_RECIPIENT);
  const [otherEmail, setOtherEmail] = useState("");
  const disabledTitle = previewMode ? "Available once a production Supabase project is connected" : undefined;

  const selectedMember = companyMembers.find((m) => m.userId === selectedUserId) ?? null;
  const recipient = selectedMember
    ? { type: "User" as const, id: null, name: selectedMember.email, address: selectedMember.email }
    : { type: "Email" as const, name: otherEmail || "Recipient", address: otherEmail || null };
  ...
  return (
    <div className="rounded-vf-md border border-vf-paper-border p-4">
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2 border-t border-vf-paper-border pt-3">
        <Select aria-label="Recipient" value={selectedUserId} onChange={(e) => setSelectedUserId(e.target.value)} className="max-w-xs">
          {companyMembers.map((m) => (
            <option key={m.userId} value={m.userId}>{m.email}</option>
          ))}
          <option value={OTHER_RECIPIENT}>Other (enter email)…</option>
        </Select>
        {!selectedMember && (
          <Input placeholder="Recipient email" value={otherEmail} onChange={(e) => setOtherEmail(e.target.value)} className="max-w-xs" />
        )}
        <SendCommunicationButton
          key={`${selectedUserId}:${otherEmail}`}
          companyId={companyId}
          module="Auditor"
            ...
            <div>
              ...
              {open.length === 0 ? (
                ...
                <div className="flex flex-col gap-3">
                  {open.map((f) => (
                    <FindingCard key={f.id} finding={f} companyId={companyId} companyMembers={companyMembers} previewMode={previewMode} />
                  ))}
                </div>
              )}
            </div>
