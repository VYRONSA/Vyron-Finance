...
export function OrgMasterDataTab({
  ...
  // Finding #168 (RC-3) — mirrors the established `ConfirmActionRow`/
  // `useConfirmTarget` pattern; only the destructive Deactivate direction
  // is confirmed, not Reactivate.
            ...
            {items.map((item) =>
              editingId === item.id ? (
                ...
                <TableRow key={item.id}>
                  ...
                  <TableCell className="text-right">
                    {deactivateConfirm.isConfirming(item.id) ? (
                      <ConfirmActionRow
                        message={`Deactivate ${item.name}?`}
                        loading={loading}
                        tone="danger"
                        onConfirm={() => toggleActive(item.id, item.isActive)}
                      ...
                      <div className="flex justify-end gap-2">
                        ...
                        <Button
                          ...
                          title={disabledTitle}
                          onClick={() => (item.isActive ? deactivateConfirm.request(item.id) : toggleActive(item.id, item.isActive))}
                        >
