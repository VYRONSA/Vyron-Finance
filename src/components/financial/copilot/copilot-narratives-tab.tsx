...
function NarrativeCard({ narrative, companyId, previewMode }: { narrative: CopilotNarrative; companyId: string; previewMode: boolean }) {
  const router = useRouter();
  const [expanded, setExpanded] = useState(false);
  const [loading, setLoading] = useState(false);
  // Finding #171 (RC-16/E13) — narratives previously had only an
  // Expand/Hide toggle, no way to remove one.
  ...
  async function deleteNarrative() {
    setLoading(true);
    try {
      const res = await fetch(`/api/companies/${companyId}/copilot/narratives/${narrative.id}`, { method: "DELETE" });
    ...
    <div className="rounded-vf-md border border-vf-paper-border p-4">
      <div className="flex items-center justify-between gap-2">
        ...
        {deleteConfirm.isConfirming(narrative.id) ? (
          <ConfirmActionRow message="Delete this narrative?" loading={loading} tone="danger" onConfirm={deleteNarrative} onCancel={deleteConfirm.cancel} />
        ) : (
          <div className="flex items-center gap-2">
            <Button variant="subtle" size="sm" onClick={() => setExpanded((v) => !v)}>
              {expanded ? "Hide" : "View"}
            </Button>
            <Button variant="subtle" size="sm" disabled={previewMode || loading} title={disabledTitle} onClick={() => deleteConfirm.request(narrative.id)}>
              Delete
            </Button>
          </div>
        )}
    ...
    <div className="flex flex-col gap-4">
      ...
      {narratives.length === 0 ? (
        ...
        <div className="flex flex-col gap-3">
          {narratives.map((n) => (
            <NarrativeCard key={n.id} narrative={n} companyId={companyId} previewMode={previewMode} />
          ))}
        </div>
      )}
    </div>
