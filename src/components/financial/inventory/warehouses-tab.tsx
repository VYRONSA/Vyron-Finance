...
export function WarehousesTab({ companyId, warehouses, previewMode }: { companyId: string; warehouses: Warehouse[]; previewMode: boolean }) {
  ...
  const [error, setError] = useState<string | null>(null);
  // Finding #232 (RC-3) — mirrors the established `ConfirmActionRow`/
  // `useConfirmTarget` pattern; only the destructive Deactivate
  // direction is confirmed. The real stock-level block (#232's other
  // half) lives server-side in `warehouse-service.ts::setWarehouseActive`
  // — its `ValidationError` surfaces here via the normal error path.
  const deactivateConfirm = useConfirmTarget<number>();
            ...
            {warehouses.map((w) => (
              <TableRow key={w.id}>
                ...
                <TableCell className="text-right">
                  {deactivateConfirm.isConfirming(w.id) ? (
                    <ConfirmActionRow
                      message={`Deactivate ${w.name}?`}
                      loading={loadingId === w.id}
                      tone="danger"
                      onConfirm={() => runAction(w.id, "set-active", { isActive: false })}
                      onCancel={deactivateConfirm.cancel}
                    />
                  ) : (
                    <div className="flex flex-wrap justify-end gap-1.5">
                      ...
                      <Button
                        variant="subtle"
                        size="sm"
                        disabled={previewMode || loadingId === w.id}
                        title={disabledTitle}
                        onClick={() => (w.isActive ? deactivateConfirm.request(w.id) : runAction(w.id, "set-active", { isActive: true }))}
                      >
