...
import { stockItemOptions } from "@/lib/account-picker-options";
    ...
    <div className="rounded-vf-md border border-vf-paper-border p-4">
      ...
      <div className="mt-4 flex flex-col gap-2">
          ...
          return (
            <div key={i} className="flex flex-wrap items-center gap-2">
              <div className="min-w-[220px] flex-1">
                {/* Finding #167 — searchable Combobox instead of a plain,
                    unfiltered Select — the only Stock Item picker in a
                    Sales/Purchasing/Inventory document line. */}
                <Combobox aria-label={`Stock item for line ${i + 1}`} value={line.stockItemId || null} options={activeStockItemOptions} placeholder="Search stock item…" onCommit={(val) => updateLine(i, { stockItemId: val ?? 0 })} />
              </div>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {/* Finding #059 — a real stock take covers hundreds of items;
              adding+selecting one row at a time was impractical. Loads
              every item whose default warehouse is the one being
              counted, pre-filled with its current system quantity as a
              starting count (still fully editable per line). */}
          <Button
            variant="subtle"
            size="sm"
            className="w-fit"
            ...
            onClick={() => {
              const warehouseItems = stockItems.filter((s) => s.defaultWarehouseId === warehouseId);
              if (warehouseItems.length === 0) return;
              setLines(warehouseItems.map((s) => ({ stockItemId: s.id, countedQuantity: String(s.quantityOnHand) })));
            }}
          >
            Load all items in this warehouse
          </Button>
        </div>
      </div>
