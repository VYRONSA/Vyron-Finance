      ...
      const body =
        mode === "create"
          ? {
              ...
              glAccount: values.glAccount.trim(),
            }
          : {
              accountName: values.accountName.trim(),
              bankName: values.bankName.trim(),
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-5" noValidate>
      ...
      <div className="grid grid-cols-2 gap-4">
        {/* Finding #141 — no canonical account-type list exists anywhere
            else in the codebase (only this same free-text field), so a
            hard-restricted <Select> risks rejecting a legitimately
            different bank's wording. A <datalist> gives structure —
            common types suggested — without blocking anything unlisted. */}
        ...
        <Field label="Branch Code" htmlFor="branch">
          <Input id="branch" inputMode="numeric" pattern="[0-9]*" value={values.branch} onChange={(e) => set("branch", e.target.value.replace(/[^0-9]/g, ""))} placeholder="250655" />
        </Field>
      </div>
      ...
      <Field label="GL Account" htmlFor="glAccount">
        <Combobox
          value={values.glAccount || null}
          options={glOptions}
          placeholder="1000 — Bank Current Account"
          aria-label="GL Account"
          onCommit={(val) => set("glAccount", val ?? "")}
        />
        <p className="text-xs text-vf-ink-faint">
          The control account Journal generation and Cashbook capture use for this account&apos;s side of the entry.
          Required before this account can be used to generate journals or capture Cashbook entries — leaving it
          blank will block those actions until a GL Account is set.
        </p>
      </Field>
