 ...
 *
 * Master Implementation Tracker — Programme 2, Root Cause RC-7, Finding
 * #095. GL Account is now a validated `Combobox` (matching every other
 * GL field this root cause covers). The remaining raw-ID fields
 * (Customer/Supplier/Branch/Department/Cost Centre/Project/Inventory
 * Item) are outside RC-7's own scope (GL/Customer/Supplier pickers) —
 * only GL Account is addressed here; a real fix for the others would
 * need each of those entity lists threaded into this page too. */
export function SplitTransactionForm({ companyId, transactionId, amount, chartOfAccounts, previewMode, onDone }: { companyId: string; transactionId: number; amount: number; chartOfAccounts: ChartOfAccount[]; previewMode: boolean; onDone: () => void }) {
      ...
      {lines.map((line, i) => (
        <div key={i} className="rounded-vf-sm border border-vf-paper-border/70 bg-vf-paper-alt/40 p-2.5">
          <div className="flex flex-wrap items-end gap-2">
            ...
            <Combobox
              aria-label={`Split ${i + 1} GL account`}
              className="w-40"
              value={line.glAccount || null}
              options={glOptions}
              placeholder="GL Account"
              onCommit={(val) => updateLine(i, "glAccount", val ?? "")}
            />
