...
function PaymentsTable({ payments }: { payments: SupplierMatchingWorkspaceData["payments"] }) {
  const { search, setSearch, sort, toggleSort, result } = useSearchAndSort(payments, (p) => p.paymentNumber, "paymentDate");
  ...
  return (
    <div className="flex flex-col gap-2">
      <Input aria-label="Search payments" placeholder="Search payment number…" value={search} onChange={(e) => setSearch(e.target.value)} className="w-64" />
      {result.length === 0 ? (
        <EmptyState title="No payments match this search." description="Clear the search to see every payment." />
      ) : (
        <Table>
          <TableHead>
            <tr>
              <SortableHeadCell field="paymentNumber" sort={sort} onSort={toggleSort}>Payment</SortableHeadCell>
              <SortableHeadCell field="paymentDate" sort={sort} onSort={toggleSort}>Date</SortableHeadCell>
...
function DocumentsPanel({ companyId, supplierIds, supplierNameById }: { companyId: string; supplierIds: number[]; supplierNameById: Map<number, string> }) {
          ...
          >
            {supplierIds.map((id) => (
              <option key={id} value={id}>
                {supplierNameById.get(id) ?? `Supplier #${id}`}
              </option>
            ))}
          </select>
        </div>
...
function StatementPanel({ companyId, data, previewMode }: { companyId: string; data: SupplierMatchingWorkspaceData; previewMode: boolean }) {
  ...
  const supplierNameById = useMemo(() => new Map(data.suppliers.map((s) => [s.id, s.name])), [data.suppliers]);

  // Finding #216 (RC-5) — the selected supplier survives a refresh and
  // participates in Back/Forward instead of silently resetting.
  const [supplierIdParam, setSupplierIdParam] = useUrlParam("statementSupplier", "");
  const supplierId = supplierIdParam ? Number(supplierIdParam) : (supplierIds[0] ?? null);
  const setSupplierId = (id: number) => setSupplierIdParam(String(id));
...
export function SupplierMatchingTab({ companyId, data, previewMode }: { companyId: string; data: SupplierMatchingWorkspaceData; previewMode: boolean }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {active === "Purchase Orders / GRNs" && <DocumentsPanel companyId={companyId} supplierIds={supplierIds} supplierNameById={supplierNameById} />}
