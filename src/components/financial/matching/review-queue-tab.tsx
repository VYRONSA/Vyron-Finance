...
export function ReviewQueueTab({
  ...
  // Finding #238 (RC-5) — both filters survive a refresh and participate
  // in Back/Forward. `typeFilter` (a discrete select) syncs immediately;
  // `search` (free text) syncs debounced so typing doesn't fire a URL
  // update — and the router work that comes with it — on every keystroke.
  const [typeFilter, setTypeFilter] = useUrlParam("queueType", ALL_TYPES);
  const [urlSearch, setUrlSearch] = useUrlParam("queueSearch", "");
  const [search, setSearch] = useState(urlSearch);
  useEffect(() => {
    const timeout = setTimeout(() => setUrlSearch(search), 400);
    ...
    <div className="flex flex-col gap-4">
      ...
      {queueHasMore && (
        <p className="text-xs text-vf-warning">
          Showing the most recent {queue.filter((i) => i.itemType === "BankTransaction").length} unmatched bank transactions — refine filters or resolve items to see more.
        </p>
      )}
              ...
              <div key={item.id} className="rounded-vf-md border border-vf-paper-border p-3">
                ...
                {transactionId !== null && splittingId === transactionId && (
                  <div className="mt-3 border-t border-vf-paper-border pt-3">
                    <SplitTransactionForm companyId={companyId} transactionId={transactionId} amount={Math.abs(item.amount ?? 0)} chartOfAccounts={chartOfAccounts} previewMode={previewMode} onDone={() => setSplittingId(null)} />
                  </div>
                )}
              </div>
            );
