...
export function GlInquiryTab({
  ...
  // Master Implementation Tracker — Epic E1, Finding #244. Every filter
  // in this row (not just Search) shares one "Apply" action — a
  // deliberate batch-filter design, not an isolated inconsistency in the
  // search box alone, since GL Inquiry is the app's most heavily-filtered,
  // scale-sensitive screen (real server-side keyset pagination, not a
  // client-side live-filter over an already-loaded list like most other
  // search boxes in the app). Switching just Search to debounced live
  // querying would make it behave differently from its four sibling
  // filters in the same row — Enter-to-Apply on any of them keeps the
  // "one batch, one Apply" model intact while still letting a keyboard
  // user trigger it without reaching for the mouse.
  function handleFilterKeyDown(e: KeyboardEvent<HTMLInputElement>) {
    if (e.key === "Enter") runQuery(null, false);
  }
    ...
    <div className="flex flex-col gap-4">
      <div className="flex flex-wrap items-end gap-2">
        <div className="w-36">
          ...
          <Input id="gl-date-from" type="date" disabled={previewMode} value={dateFrom} onChange={(e) => setDateFrom(e.target.value)} onKeyDown={handleFilterKeyDown} />
        </div>
        <div className="w-36">
          <label className="mb-1 block text-xs font-medium text-vf-ink-soft" htmlFor="gl-date-to">To</label>
          <Input id="gl-date-to" type="date" disabled={previewMode} value={dateTo} onChange={(e) => setDateTo(e.target.value)} onKeyDown={handleFilterKeyDown} />
        </div>
        <div className="w-48">
        ...
        <div className="w-40">
          ...
          <Input id="gl-reference" disabled={previewMode} value={reference} onChange={(e) => setReference(e.target.value)} onKeyDown={handleFilterKeyDown} />
        </div>
        <div className="min-w-[180px] flex-1">
          ...
          <Input
            ...
            placeholder="Description or reference — press Enter or Apply…"
          />
        </div>
      ...
      <p className="text-xs text-vf-ink-faint">
        These filters query the full General Ledger history and apply together — press Enter in any field, or click Apply, to run them.
      </p>
