  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-wrap items-center gap-2">
         ...
         * import requires." Served from a route so the header source
         * (`chart-of-accounts-service.ts`) never has to be imported into
      ...
      <p className="text-xs text-vf-ink-faint">
        Download the template, complete it, and upload it here. <span className="font-medium text-vf-ink-soft">Account Code, Description, Account Type, and Normal Balance</span> are required.
        {" "}Parent Account, if provided, must match another Account Code in the same import or already on file.
      </p>
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <Field label="Parent Account" htmlFor="coa-parent">
              ...
              <Select id="coa-parent" value={form.parentAccountId} onChange={(e) => setForm({ ...form, parentAccountId: e.target.value })}>
                ...
                {accounts.filter((a) => a.id !== editingId && a.isActive).map((a) => (
