...
export function Sparkline({
  ...
  // Finding #001/#012 (RC-16/E12) — `i / (data.length - 1)` divides by
  // zero (NaN, for every point) when `data` has exactly one element —
  // a real case, not hypothetical (the "Automation Score" chip's own
  // recovery trend is a single point outside preview mode). A single
  // point has no line to draw a slope across, so it's centered instead.
