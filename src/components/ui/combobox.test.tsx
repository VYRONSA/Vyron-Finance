...
describe("Combobox — named lookup scenarios", () => {
  ...
  // Phase 26G — the "browse without typing" view used to share its
  // 50-item cap with the actively-searching view, silently hiding
  // anything past position 50 in a list opened cold (no text typed) —
  // confirmed as the exact cause of "the GL account list looks limited"
  // once a real chart of accounts grew past 50 rows (migration 0086).
  it("shows more than 50 options when the list is large and nothing has been typed yet (was silently capped at 50)", () => {
    const manyOptions: ComboboxOption<string>[] = Array.from({ length: 76 }, (_, i) => {
      const code = String(1000 + i);
      return { value: code, label: code, sublabel: `Account ${i}`, searchText: `${code} Account ${i}` };
    });
    render(<Combobox value={null} options={manyOptions} onCommit={() => {}} aria-label="Account" />);
    fireEvent.focus(screen.getByRole("combobox", { name: "Account" }));
    // The 76th (last, highest-numbered) option must be reachable without typing anything.
    expect(screen.getByText(manyOptions[75].label)).toBeInTheDocument();
  });
});
