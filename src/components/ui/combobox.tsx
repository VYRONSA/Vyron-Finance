...
export type ComboboxProps<T extends string | number> = {
  ...
  "aria-label"?: string;
  className?: string;
  /** Phase 47 — this component's own default input styling
   * (`bg-transparent`/`border-transparent` until focused) was built for
   * embedding inside a dense table cell (see the doc comment above),
   * where blending into the surrounding cell is correct. Standalone form
   * usage (a filter row, a normal field next to `Input`/`Select`
   * siblings) needs the opposite: a real value can render with fully
   * correct text and state (confirmed — this was never a state bug) while
   * still reading as empty/disabled next to bordered, solid-background
   * siblings, since there's no visible affordance that a value is even
   * present. This lets a caller override the input's own classes (e.g.
   * to `FIELD_BASE` from `@/components/ui/input`) without touching the
   * many existing dense-cell callers, which simply omit it. */
...
// Phase 26G — the empty-query "browse without typing" view used to share
// this SAME 50-item cap with the actively-searching (fuzzy-matched) view.
// That's fine when narrowing a large list down to relevant matches, but
// wrong when just opening the list cold: options are rendered in
// caller-supplied order (chart-of-accounts callers sort by account code
// ascending), so once a list crosses 50 items the LAST (highest-numbered)
// entries are silently invisible until the user actively types something
// — confirmed as the exact cause of "the GL account list looks limited"
// after the chart grew past 50 accounts (Phase 26F's migration 0086).
// 500 comfortably covers any real chart/supplier/customer list this app
// seeds or imports without meaningfully hurting render cost (still a
// plain scrollable `<ul>`, no virtualization needed at this size).
...
export function Combobox<T extends string | number>({
  ...
  const filtered = useMemo(() => {
    ...
    if (!query.trim()) {
      results = options.slice(0, MAX_BROWSE_RESULTS);
    } else {
