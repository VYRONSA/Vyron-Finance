...
const nextConfig: NextConfig = {
  ...
  // Phase 24A — `puppeteer-core` + `@sparticuz/chromium` (server-generated
  // PDFs). Same category of problem as `pdfjs-dist` above (a package that
  // ships real binary/native content Vercel's deployment file-tracer
  // needs explicit help finding), so it gets the exact same two-part
  // treatment rather than a new pattern: `serverExternalPackages` so the
  // bundler doesn't try to inline `@sparticuz/chromium`'s packed binary
  // into the JS bundle (which would corrupt it), and
  // `outputFileTracingIncludes` so Vercel's deployment trace includes it
  // anyway despite that exclusion.
  serverExternalPackages: ["pdfjs-dist", "@napi-rs/canvas", "puppeteer-core", "@sparticuz/chromium"],
  outputFileTracingIncludes: {
    "/*": ["node_modules/pdfjs-dist/**/*", "node_modules/@napi-rs/**/*", "node_modules/@sparticuz/chromium/**/*"],
  },
};
