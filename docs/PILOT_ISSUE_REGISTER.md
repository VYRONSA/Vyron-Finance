...
*Round status note: `v1.0.0-pilot1` was tagged 2026-08-06 reflecting Round 1's state at that moment, but the Board's Final Outstanding Requirement (VR-015) and Additional Requirement (VR-016), both added the same day, mean the round is not actually fully closed — Phase 9 (PDF Bank Statement Import) remains open for 9 of 10 banks pending real sample statements or an explicit Board deferral decision, and Purchase Processing multi-line capture (VR-016) is code-complete pending migration `0059`. The tag itself is frozen and will not be modified; a future tag will mark the round's true close once VR-015/VR-016 resolve.*
...
- **Priority**: High — the original Phase 9 business requirement (production PDF import for 10 named SA banks) remains unmet for 9 of 10 banks, pending real samples
- **Status**: Complete for the framework/review/validation/error-handling work and the FNB parser's code-complete-and-live-verified-on-synthetic-data state; genuinely Open for the other 9 banks (no samples yet) and for FNB's final "validated" status (needs the real file)
...
### VR-016 — Purchase Processing: Bills/Credit Notes/Debit Notes had no multi-line capture (Product Review Board's "Additional Requirement: Purchase Processing")
- **Description**: `ae_imported_bills` has never had a line-items table (imported bills never had one either) — a Purchasing-entered Bill/Credit Note/Debit Note was a single flat record: one net amount, one GL account, one VAT treatment. The Board's Additional Requirement is explicit: unlimited lines, each independently allocated to a GL account, VAT code, cost centre, project, department, with its own description/quantity/unit cost/discount, captured entirely before posting (matching the spreadsheet-style capture philosophy already established for Cashbook/Opening Balances). Added a new `ae_purchase_bill_lines` child table (migration `0059`, **not yet applied**), a `BatchEntryGrid`-based capture screen (real GL/VAT/cost-centre/project/department selects, running Subtotal/VAT/Total footer) replacing the old flat form, and `buildMultiLineBillJournal` — the posting-time change needed because a multi-line bill can carry more than one GL account and more than one VAT rate, which the existing single-amount posting-rule call couldn't express (posts one debit line per distinct line GL account plus aggregated VAT Input/Creditors Control lines). The legacy single-amount path (`createBillFromOrder`, imported bills) is untouched and still works exactly as before.
- **Priority**: High — explicit Board requirement bundled into the same Certification Requirement as VR-015's PDF work
- **Status**: Code-complete, unit-tested, `tsc`/`eslint`/build clean; **live verification blocked on migration `0059`** — confirmed live: the bill-creation request reaches the database and fails honestly with "table not found," not a code-path error, isolating exactly what's pending
- **Version Target**: 1.0 (Pilot Round 1) — Additional Requirement
- **Date Found**: 2026-08-06
- **Date Fixed**: Code-complete 2026-08-06; live verification pending
- **Verified By**: `tsc`/`eslint`/`vitest` (9 new unit tests for `computeBillLine`/`lineAmountFor`)/`npm run build` all clean. Live: company/supplier/chart-of-accounts/VAT-treatment/posting-rule setup and the bill-creation request itself confirmed reaching the real API and service layer correctly — the only failure is the missing table, which migration `0059` resolves.
