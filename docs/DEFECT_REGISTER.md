...
- **Status: FIXED** — resolved by D-018's same migration (`0054`).
...
**D-032 was not fixed** — this pass supplied the first live confirmation that its predicted failure mode actually occurs (see D-032's own entry for the duplicate-company evidence), and produced a dedicated architecture review, `docs/ARCHITECTURE_REVIEW_COMPANY_BILLING.md`, recommending a structural fix. The Board accepted that recommendation in principle but deferred it to Version 1.1 (`docs/ADR_001_COMPANY_CREATION_BILLING_DECOUPLING.md`) — **do not treat D-032 as resolved; it remains open for RC1 and Version 1.0.**
