...
| FNB PDF import — framework, review screen, validation, duplicate detection, metadata storage, Banking Rules integration, workflow plumbing | **Certified** | 18/18 live checks against the real database (§7); 3 independent full-pipeline runs across this round with zero regressions |
| FNB PDF import — parser field-extraction accuracy against the actual statement | **Not certified — blocked** | The actual PDF file is not accessible to this session; cannot be certified without it, per the Board's own instruction not to assume correctness |
...
**One real defect was found and fixed during this certification round**: no editing capability existed for a Draft Bill's or Purchase Order's lines at all — a direct miss against the certification's own "Correct editing" requirement, found only because the pass was run live rather than assumed. Fixed the same session, per the Board's stop-fix-retest instruction, and re-verified — see VR-017.

**What closes this out**: making the two real FNB PDF files genuinely accessible to this session (a file path this session can read, not a description of having supplied them) so their extracted values can be compared line-by-line against the actual documents, as the certification instruction requires.
...
95 new tests, all passing, on top of the pre-existing suite (now 1254/1254, up from 1168):
...
**FNB PDF Import workflow — 18/18 checks passing (VR-015, third independent full-pipeline run, zero regressions):**
...
- **What this cannot certify**: the actual FNB file's real field-extraction accuracy — this run used a synthetic statement matching the real one's text shape, since the actual PDF file remains inaccessible to this session (see VR-015's final follow-up).

## 8. Remaining Recommendations

1. **Make the two real FNB PDF files genuinely accessible to this session** (a readable file path, not a description of having supplied them) — the one remaining step to compare extracted values against the actual documents and move FNB from `implemented-unvalidated` to `validated`.
2. **Supply real, redacted sample bank statements** for the remaining 9 banks (see Phase 9's own table and `REQUIRED_SAMPLE_STATEMENTS`), or have the Board formally defer specific banks to Pilot Review Round 2, per the Final Outstanding Requirement's own stated final-acceptance condition.
3. **Add unit tests** for `correctPostedBankOpeningBalanceIfNeeded`, `bulkUpsertGlOpeningBalances`, a regression case for VR-014 (`applyRuleActions` with no prior supplier/customer match), and for the new `updateBillLines`/`updateOrderLines` editing paths, per §6.
...
6. **Audit the pre-existing "Apply Rule" bulk action and "Run Rule Engine Now" button** for any other transactions that may have silently failed to resolve prior to the VR-014 fix — the defect predates this round and shared the same code path.
7. **Consider migrating the Journals capture screen onto `BatchEntryGrid`** — Journals already support real multi-line debit/credit capture (a bespoke line array with Add/Remove and a live-balance check), so no functional gap exists there, but it's the one remaining multi-line screen not on the shared framework Cashbook/Opening Balances/Purchasing (Bills and Purchase Orders) now all use. Not required by any Board directive — flagged as a consistency opportunity, not a defect.
8. **Board suggestion, explicitly not a Round 1 blocker**: replace the Import Centre's two separate panels (Bills/Credit Notes, Bank Statement) with a single Import Wizard — choose the document type first, then upload. A worthwhile usability enhancement for a future release; noted here so it isn't lost, not scheduled against this round.
9. Everything from the original Phase 1–3 report's own recommendations (the `status !== 'onboarding'` go-live proxy, the `requireApproval`-as-supervisor-authentication interpretation) still stands, unchanged this round.
...
| `npx vitest run` | 1254/1254 tests, zero regressions |
| `npm run build` | Exit 0, zero prerender errors |
| Live verification | Company onboarding, Opening Balances grid, Cashbook batch capture, Master Data edits across 5 modules, and the Final Certification round's four areas all confirmed live: Purchase Processing 28/28, Bank Opening Balances 16/16, Banking Rules 16/16, PDF import workflow plumbing 18/18 (§7) |
| Phase 9 (PDF Bank Statement Import) final acceptance | **Partially met** — the feature is genuinely accessible and live-verified end to end (upload, review, correction, confirmation, Banking Rules execution, duplicate detection on re-upload all work, re-confirmed across 3 independent full-pipeline runs); FNB has a real parser (`implemented-unvalidated`, Board-confirmed correct); 9 of 10 banks remain `awaiting-validation`. Needs the actual FNB file (still not accessible to this session) to reach `validated`, real sample statements for the remaining 9 banks, or an explicit Board deferral decision, see Phase 9's own section and §8 |
